...
class RakeloadingModel extends Model
{
	...
	public function FNRNOGet($plantIds){
        ...
        $builder =  $builder->where('VECHICAL_STATUS IN (0,2)');
	...
	public function getWHplantListSDI($plantFilter)
	{
		...
		$cnd.="VEHICLE_ARRIVED = '0' and purchase_info_id = 0 and WB_QTY NOT IN(' ') $plants";
	...
	public function gateInOutInfo($plantFilter)
	{
		...
		$Sql = "SELECT  id as value, vehicleNo as label
				...
				WHERE masterPlantId IN ($plantId) AND moduleStatusId = 0 AND movementType = 2 AND moduleType NOT IN (12,15,21,14)";
		$builder = $this->db->query($Sql);		
		return  $builder->getResultArray();
	}
	...
	public function Supplier_Vehicle_ByID($id){	
		$builder = $this->db->table("supplier_vehical_info");
		$builder = $builder->select("SUPPLIER_ID,LINE_ITEM");
		$builder =  $builder->where("VEHICLE_ARRIVED", '0');
		$builder =  $builder->where("SUP_VE_REFID","$id");
		$builder =  $builder->distinct()->get()->getResultArray();
		$supplier_vehical_info = $builder;
		// print_r($builder[0]['SUPPLIER_ID']);exit;

		$builders = $this->db->table("supplier_dispatch_info");
		$builders = $builders->select('*');
		$builders =  $builders->where("SD_REFID=",$supplier_vehical_info[0]['SUPPLIER_ID']);
		$builders = $builders->distinct()->get()->getResultArray();

		return ["supplier_vehical_info"=>$supplier_vehical_info,"supplier_dispatch_info"=>$builders];
	...
	public function FNRNOBasedCount($fnr_no) {
		...
		$process_cancel = $this->db->table("purchase_info")->where("(FNR_NO='$fnr_no' AND VECHICAL_STATUS='18')")->countAllResults();
		$process_cancel1 = $this->db->table("purchase_info")->where("(FNR_NO='$fnr_no' AND VECHICAL_STATUS>'25')")->countAllResults();
