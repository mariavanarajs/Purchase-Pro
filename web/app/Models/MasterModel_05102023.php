...
public function getMaster_vendorDetailsById($Id) {
 $fetchsql = "SELECT `Id`, `Code`, `Name`, `Category`, `InsBy`, `InsDt`, `ModBy`, if(`ModDt` is null,'',ModDt) as ModDt, `RecStatus` FROM `master_vendor` WHERE RecStatus='1' and Id='$Id'";
 $builder =  $this->db->query($fetchsql);
 return  $builder->getResultArray();
}
...
public function getWeighmentImages($postData) {  
  ...
  $sapUrlHelperService = new SapUrlHelper();
    $fileLocation = "/var/www/purchasepro/sapfilesharepurchasepro";
  if(count($cameraData) > 0){	
    ...
    $imageStoragePath = $storagePath.$postData->vaNumber;
    ...
    foreach($cameraData as $key => $dataValue){
      ...
      $imagePath = str_replace("/var/www/purchasepro/sapfilesharepurchasepro/", "https://purchasepro.nagamills.com/", $result);
