...
class Loadingunloadingcost extends Model
{
	...
	public function Unloading_cost($data){
		...
		$builder = $builder->join('master_vendor', "master_vendor.Name = gateout_info.UnloadVendorName AND master_vendor.RecStatus = 1 AND master_vendor.Category = 'Unloading Vendor'", 'inner');
		$builder =  $builder->where($cnd);
	...
	public function IASUnLoading($data){
		...
		ROUND(intrastate_gateout_info.WbNetWt/1000,3) as total_weight,
		intrastate_gateout_info.UnloadingChargePerTon as loading_charge,
		intrastate_gateout_info.UnLoadingVendor as Name,
		ROUND((intrastate_gateout_info.WbNetWt/1000)*(intrastate_gateout_info.UnloadingChargePerTon),0) as value,
		ROUND((intrastate_gateout_info.WbNetWt/1000)*(intrastate_gateout_info.UnloadingChargePerTon),0) as confirm_value,
		...
		// $builder = $builder->join('pp_to_sap', 'pp_to_sap.ZVA_NUMBER = purchase_info.ZVA_NUMBER and pp_to_sap.isupdate = 0', 'inner');
		$builder = $builder->join('master_plant', "master_plant.WERKS = purchase_info.WERKS", 'inner');
		//$builder = $builder->join('intrastate_gateout_info', "intrastate_gateout_info.ReceivingArrivalId = purchase_info.PI_REFID and intrastate_gateout_info.UnloadingChargePerTon != 0 and and intrastate_gateout_info.ReceivingArrivalId != ''", 'left');
		//$builder = $builder->join('intrastate_warhouse_dispatch_info', "intrastate_warhouse_dispatch_info.VehicleArrivalId = intrastate_gateout_info.EmptyVehicleArrivalId", 'left');
		...
		$builder = $builder->join('master_vendor', "master_vendor.Name = intrastate_gateout_info.UnLoadingVendor AND master_vendor.RecStatus = 1 AND master_vendor.Category = 'Unloading Vendor'", 'inner');
		$builder =  $builder->where($cnd);
		return  $builder->groupBy('purchase_info.PI_REFID')->get()->getResultArray();
	...
	public function Loading_Cost($data){
		// print_r($data);exit;
		$PlantId = $data->Plant;
		$Vendor_Name = $data->Vendor_Name;
		$po_number = $data->po_number;
		...
		$vehicle_type=$data->vehicle_type == 'Purchase' ? '7' : 0;
		  ...
		  if(isset($data->fromdate) and isset($data->todate) and isset($data->Vendor_Name)and isset($data->company_name)and isset($data->vehicle_type))
		  {
			  ...
			  $Filter_VendorName = rtrim($Filter_VendorName, ",");
			  $cnd .= "purchase_info.DateAdded >= '$data->fromdate' and purchase_info.DateAdded <= date_add('$data->todate', INTERVAL 1 DAY) and purchase_info.VECHICAL_STATUS = '$vehicle_type' and gateout_info.miro_status = 0 and purchase_info.MIGO_NUM != '' and master_plant.plant_subdivision='$data->company_name' and master_vendor.Code IN($Filter_VendorName) and gateout_info.LoadingCharge > 0 AND purchase_info.VEHICLE_TYPE IN ('Truck','TRUCK','Container','CONTAINER' AND purchase_info.SCREEN_TYPE = 'SDI')";
		  }
		...
		$builder = $this->db->table("purchase_info");
		$builder = $builder->select("gateout_info.GO_REFID,
		gateout_info.GO_REFID as index,
		purchase_info.TRUCK_NO as vehicle_no,
		purchase_info.ZPO_NUMBER as po_number,
		purchase_info.PO_LINE_ITEM as po_line_item,
		ROUND(SUM(gateout_info.wb_net_wt/1000),3) as total_weight,
		gateout_info.LoadingCharge as loading_charge,
		master_vendor.Name,
		ROUND(SUM((gateout_info.wb_net_wt/1000)*gateout_info.LoadingCharge),0) as value,
		ROUND(SUM((gateout_info.wb_net_wt/1000)*gateout_info.LoadingCharge),0) as confirm_value,
		purchase_info.VEHICLE_TYPE as vehicle_type,
		IF(purchase_info.VECHICAL_STATUS != '','Loading','') as Type,
		purchase_info.WERKS as plant_id,
		DATE_FORMAT(purchase_info.DateAdded, '%d-%m-%Y')as DateAdded,
		purchase_info.MIGO_NUM,
		purchase_info.ZVA_NUMBER");
		...
		$builder = $builder->join('master_plant', 'master_plant.WERKS = purchase_info.WERKS', 'inner');
		$builder = $builder->join('gateout_info', 'gateout_info.purchase_info_id = purchase_info.PI_REFID', 'inner');
		$builder = $builder->join('master_vendor', 'master_vendor.Id = gateout_info.LoadingVendorID', 'inner');
		$builder =  $builder->where($cnd);
		//return  $builder->groupBy('purchase_info.PI_REFID')->get()->getResultArray();
	    return  $builder->groupBy(['purchase_info.PI_REFID'])->get()->getResultArray();
	...
	public function Loading_vendor($data){
	...
	public function Load_Unload_Payment_Index($data){
		$builder = $this->db->table("loading_unloading_payment");
		$builder = $builder->select("loading_unloading_payment.*,master_vendor.Code,master_vendor.Name,master_vendor.tds_code,master_vendor.tds_name,IF(loading_unloading_payment.status=1,'WH MG Approve',IF(loading_unloading_payment.status=2,'Acc. Confirmation',IF(loading_unloading_payment.status=3,'Acc MG Approve',IF(loading_unloading_payment.status=4,'Miro Completed',IF(loading_unloading_payment.status=5,'Rejected',IF(loading_unloading_payment.status=6,'Accounts MG Rejected',IF(loading_unloading_payment.status=7,'Miro Reversed',IF(loading_unloading_payment.status=8,'WH Incharge Correction','')))))))) as Waiting_at,DATE_FORMAT(loading_unloading_payment.created_at, '%d-%m-%Y') AS created_at");
	...
	public function Load_Purchase_Reverse($id,$status){
		$splitnumbers = $id;
		$splittedNumbe = explode(",", $splitnumbers);
		$ID = "'" . implode("', '", $splittedNumbe) ."'";
		$fetchsql = "UPDATE  `gateout_info`
		SET miro_status = $status
		where GO_REFID in ($ID)";
		$InsId=$this->db->query($fetchsql);
		return  $InsId;
    }
    ...
    public function LoadUnloadByID($id){
		
		$builder = $this->db->table("loading_unloading_payment");
		$builder = $builder->select("load_unload_info");
		$builder =  $builder->where('id',$id);
		return  $builder->get()->getResultArray();
    }
