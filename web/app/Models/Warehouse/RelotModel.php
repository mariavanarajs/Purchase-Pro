  ...
  public function getRelotDetails($RelotId){
    ...
    */
    ...
    public function getRelotReportlist($warehouseid, $plantid, $lotid, $fromdate, $todate){
      ...
      public function getWhcodeId($whcode){
        return $this->db->table('warehouse_master')
        ->select('WH_CODE')
        ->where('WH_CODE', $whcode)
        ->orWhere('WH_REFID', $whcode)
        ->get()
        ->getResultArray();            
      }
      public function getPlantId($WERKS){
        return $this->db->table('master_plant')
        ->select('WERKS')
        ->where('WERKS', $WERKS)
        ->orWhere('PLANT_NAME', $WERKS)
        ->get()
        ->getResultArray();            
      } 
