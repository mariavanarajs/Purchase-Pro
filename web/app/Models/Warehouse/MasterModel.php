...
class MasterModel extends Model
{
  ...
  public function getWHplantList($WH_CODE,$plantFilter)
  {
    if ($plantFilter != '') {
      $splitnumber = $plantFilter;
      $splittedNumbers = explode(",", $splitnumber);
      $numbers = "'" . implode("', '", $splittedNumbers) ."'";
      $plants = "a.werks IN $numbers";
    }else{
      $plants = 'a.werks NOT IN ("0")';
    }
    $cnd="";
    if(isset($WH_CODE))
    {
      $cnd.=" and b.WH_CODE='$WH_CODE' and $plants";
    }
    $Sql = "SELECT  a.ID as value,a.werks as label, 
                    a.werks, 
                    a.PLANT_NAME,
            ...
            FROM  master_plant a, 
                  warehouse_master b 
            WHERE b.WH_CODE = a.wh_code AND a.plant_subdivision IN (0,1) order by a.werks, a.PLANT_NAME";
    //echo $Sql;
  ...
  public function getWHstoragelocationBasedPlant($plantId)
  {
    $Sql = "SELECT  STORAGE_REFID as value, LGORT as label FROM  master_storage WHERE RecStatus = 1 AND plantid='$plantId'";
 ...
 public function getKeyLoanwarehouses(){
  ...
  public function getWHLotList_With_Plantid($plantid,$storagelocationId){
    ...
    $Sql = "SELECT a.lotid as value, CONCAT(b.werks,' - ',a.lotno) as label  
            FROM ngw_lot a 
            JOIN master_plant b ON a.plantid = b.ID AND b.plant_subdivision < 2
  ...
  public function getWHLotBasedSL($LocationId){
    $Sql = "SELECT a.lotno as value, lotno as label  
            FROM ngw_lot a        
            WHERE a.recstatus = 1 AND locationid='$LocationId' order by a.lotno";
  ...
  public function getWHWheatvarietyList($lotid="",$StorageLocation="")
  {
    ...
    if(c.totalcapacity<b.wheatqty,c.totalcapacity,b.wheatqty) as diff_compare_qty,c.totalcapacity, b.wheatqty,a.Segment
  ...
  public function getPlants() {
    $fetchsql = "SELECT ID as value , PLANT_NAME as label FROM `master_plant` where RecStatus='1' AND plant_subdivision < 2";
...
/*** End Relotting Model ***/
  ...
  public function getFumigationTypeData($data)
  {
    $builder = $this->db->query("select Fumigation_TypeId as value, Fumigation_Type as label from ngw_fumigation_type where Status = $data");
    return  $builder->getResultArray();
  }

   /*** Get fumigation Type Other ***/

   public function getFumigationTypeDataOther()
   {
     $builder = $this->db->query("select Fumigation_TypeId as value, Fumigation_Type as label from ngw_fumigation_type where Status = 3");
     return  $builder->getResultArray();
   }
  /*** Get fumigation vendor ***/

  public function getFumigationVendor()
  {
    $builder = $this->db->query("select Code as value, Name as label from master_vendor where Category = 'Fumigation Vendor' or 'FUMIGATION VENDOR' or 'fumigation vendor'");
    return  $builder->getResultArray();
  }
   /*** Get Reson For Delay ***/
  ...
  public function getMaster_ngw_fumigation_typeById($Id) {
   $fetchsql = "SELECT `Fumigation_TypeId`, `Fumigation_Type`,`Rate`,`Status`,`quick_gas_grm`,`quick_gas_dosing`, `InsBy`, `InsDt`, `ModBy`, `ModDt`, `RecStatus` FROM ngw_fumigation_type WHERE Fumigation_TypeId='$Id'";
   $builder =  $this->db->query($fetchsql);
   return  $builder->getResultArray();
  }
...
// echo $Sql;
  ...
  public function getMaterialCodeSegment($Segment){

    $Sql = "SELECT MaterialCode as SeedVariety,Id FROM `master_mrc_wheat_variety` where Segment='$Segment'";
    // echo $Sql;
        $builder = $this->db->query($Sql);
...
$Sql = "SELECT a.wheatqty ,b.WheatVariety,a.wheatvarietyid, a.init_lot_qty ,b.Segment
FROM `ngw_sublot` a 
...
//********************************************************************** */
    ...
    public function getLoadingVendorById($vendor_code) {
      if($vendor_code){
        $builder = $this->db->query("SELECT Id AS value, Name AS label FROM `master_vendor` WHERE Category ='Loading Vendor' AND Code LIKE '%$vendor_code%' AND RecStatus='1' ORDER BY ID DESC LIMIT 1");
        return  $builder->getResultArray();
      }else{
        $result = [
          [
    ...
    public function getFreightVendorById($vendor_code) {
      if($vendor_code){
      $builder = $this->db->query("SELECT Id AS value, Name AS label FROM `master_vendor` WHERE Category ='Last Mile Transporter' AND Code LIKE '%$vendor_code%' AND RecStatus='1' ORDER BY ID DESC LIMIT 1");
      return  $builder->getResultArray();
      }else{
        $result = [
          [
