...
class IASSendingModel extends Model{
    ...
    public function getLotBySegment($SegmentId, $PlantId){
      ...
      JOIN ngw_weeklyplan c ON a.plantid = c.fromplantid AND a.lotid = c.fromlotid AND c.Division = 'NAGA' 
        ...
        */
        /*Mohan Commented on 14-09-2022 for checking planqty-ActualMovementqty condition
            $strQuery = "SELECT DISTINCT lotid as value, lotno as label, 
                @Actual_Movement_Qty :=ifnull((SELECT SUM(IF(MovementQty IS NULL,0,MovementQty))
                 ...
                 AND wpa.MovementDate >= DATE_FORMAT(CURRENT_DATE, '%Y-%m-01')),0) AS Actual_Movement_Qty
            ...
            and (TIMESTAMPDIFF(DAY,CURRENT_DATE, date(a.nextfumigationdate)))<=ifnull(mb.FumigationValidity,30)) 
            OR (a.fumigationstatus = 9), a.wheatqty, IF(a.FumigationSkipFlag=1 and a.FumigationSkipDate>=current_date, a.wheatqty, 0)))
            ...
            */
            ...
            $strQuery = "
            Select * from (
            SELECT DISTINCT lotid as value, lotno as label, 
                @Actual_Movement_Qty :=ifnull((SELECT ROUND(SUM(IF(MovementQty IS NULL,0,MovementQty)),3)
                 FROM ngw_weeklyplan_actual wpa
                 WHERE wpa.RecStatus =1 AND (wpa.WheatVarietyId,wpa.PlantId,wpa.LotId)=
                       (c.wheatvarityid,c.fromplantid,c.fromlotid) 
                 AND wpa.MovementDate >= DATE_FORMAT(CURRENT_DATE, '%Y-%m-01')),0) AS Actual_Movement_Qty,
                 @FumigatedDaysCount:=(TIMESTAMPDIFF(DAY,CURRENT_DATE,date(a.nextfumigationdate))) as FumigatedDaysCount,
                 @Fumi_Cleared_Qty:= IFNULL(IF((@FumigatedDaysCount>0 and @FumigatedDaysCount<=ifnull(mb.FumigationValidity,30)) OR (a.fumigationstatus = 9), a.wheatqty,
          IF(a.FumigationSkipFlag=1 and a.FumigationSkipDate>=current_date, a.wheatqty, 0)),0) AS Fumi_Cleared_Qty, 
a.FumigationSkipFlag,
@QC_Cleared_Qty:= IFNULL(IF(a.nextrnddate>=current_date,a.wheatqty,
        IF(a.RndSkipFlag=1 and a.RndSkipDate>=current_date, a.wheatqty, 0)),0) AS QC_Cleared_Qty, 
@Keyloan_Cleared_Qty:= IFNULL(IF(IFNULL(a.Keyloan_Pledge_Status,'NO')='YES',a.Keyloan_Release_Qty,a.wheatqty ),0)  AS Keyloan_Cleared_Qty,
IF((c.planqty - if(@Actual_Movement_Qty is null,0,@Actual_Movement_Qty)) <= @Fumi_Cleared_Qty, '#00A000', '#A00000') FumigationClearedColor,
IF((c.planqty - if(@Actual_Movement_Qty is null,0,@Actual_Movement_Qty)) <= @QC_Cleared_Qty, '#00A000', '#A00000') QCClearedColor,
IF((c.planqty - if(@Actual_Movement_Qty is null,0,@Actual_Movement_Qty)) <= @Keyloan_Cleared_Qty, '#00A000', '#A00000') KeyloanClearedColor,
'#00D084' as ApprovalColor,'#FA6C3F' as PendingColor,
IF((c.Priority='1' ) and (
(c.planqty - if(@Actual_Movement_Qty is null,0,@Actual_Movement_Qty)) > @Fumi_Cleared_Qty or 
(c.planqty - if(@Actual_Movement_Qty is null,0,@Actual_Movement_Qty)) > @QC_Cleared_Qty or 
(c.planqty - if(@Actual_Movement_Qty is null,0,@Actual_Movement_Qty)) > IF(IFNULL(a.Keyloan_Pledge_Status,'NO')='YES',a.Keyloan_Release_Qty,a.wheatqty)
), 0, 1) AS ApprovalEnableFlag, c.MovementType                 
            ...
            JOIN ngw_weeklyplan c ON a.plantid = c.fromplantid AND a.lotid = c.fromlotid AND c.Division = 'NAGA' 
            AND (
              (c.Priority = '1' AND c.MovementType = 'MILL' AND c.STATUS = '3' AND c.plandate = DATE_FORMAT(CURRENT_DATE, '%Y-%m-01')) 

            OR (c.Status = 2 AND c.MovementType = 'GODOWN' AND c.plandate >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)))
            ) as TmpTable where MovementType='GODOWN' or ApprovalEnableFlag=1 ";
    ...
    public function getPONumber($werks){
      $strQuery = "SELECT DISTINCT PoNumber AS label, PoNumber AS value FROM intrastate_sap_to_pp WHERE SendingPlant ='".$werks."' AND PoNumber !=0 AND flag = 0";
      //echo $strQuery;exit();
    ...
    public function getReportDetails($postData){
        ...
        $StartDate = date('Y-m-d 00:00:00', strtotime($varCuurentDate));
        $EndDate = date('Y-m-d 23:59:59', strtotime($varCuurentDate));

        $start = $FilterData->FromDate;
        $end   = $FilterData->ToDate;

        $started = date('Y-m-d 00:00:00', strtotime($start));
        $ended = date('Y-m-d 23:59:59', strtotime($end));
  ...
  $strQuery = "SELECT 
    a.TruckNo, 
    a.LoadedLotNo, a.LoadedLotNo2, a.LoadedLotNo3,
    ...
    a.WbEmptyWt as Send_WbEmptyWt, a.WbLoadWt as Send_WbLoadWt, a.WbNetWt as Send_WbNetWt, 
    a.GunnyWt as Send_GunnyWt, a.GunnyLessNetWt as Send_GunnyLessNetWt,
    a.PO_Qty, a.PO_Qty2, a.PO_Qty3,a.LoadingVendor,a.LoadingChargesPerTon,

    -- BagType concatenations (ignores empty and null)
    NULLIF(CONCAT_WS(',', NULLIF(a.BagType, ''), NULLIF(a.L1_BagType2, ''), NULLIF(a.L1_BagType3, '')), '') AS L1_BagTypes,
    NULLIF(CONCAT_WS(',', NULLIF(a.L2_BagType, ''), NULLIF(a.L2_BagType2, ''), NULLIF(a.L2_BagType3, '')), '') AS L2_BagTypes,
    NULLIF(CONCAT_WS(',', NULLIF(a.L3_BagType, ''), NULLIF(a.L3_BagType2, ''), NULLIF(a.L3_BagType3, '')), '') AS L3_BagTypes,

    -- No of Bags (rounded to 2 decimals)
    ROUND(IFNULL(a.L1_NoofBags,0) + IFNULL(a.L1_NoofBags2,0) + IFNULL(a.L1_NoofBags3,0), 2) AS L1_TotalBags,
    ROUND(IFNULL(a.L2_NoofBags,0) + IFNULL(a.L2_NoofBags2,0) + IFNULL(a.L2_NoofBags3,0), 2) AS L2_TotalBags,
    ROUND(IFNULL(a.L3_NoofBags,0) + IFNULL(a.L3_NoofBags2,0) + IFNULL(a.L3_NoofBags3,0), 2) AS L3_TotalBags,

    b.ZVA_NUMBER, b.UpdateLotByName, b.FirstWeightEntryByName as  SecFirstWeightEntryByName, b.SecondWeightEntryByName as SecSecondWeightEntryByName, UI2.FIRST_NAME as SecGateInBy, b.GateOutByName as SecGateoutby,

    DATE_FORMAT(b.GateInDt, '%d-%m-%Y %H:%i') GateInDt,  
    DATE_FORMAT(b.UpdateLotDt, '%d-%m-%Y %H:%i') SECUpdateLotDt,  
    DATE_FORMAT(b.FirstWeightEntryDt, '%d-%m-%Y %H:%i') SECFirstWeightEntryDt,  
    DATE_FORMAT(b.SecondWeightEntryDt, '%d-%m-%Y %H:%i') SECSecondWeightEntryDt, 
    DATE_FORMAT(b.GATE_OUT_TM, '%d-%m-%Y %H:%i') GATE_OUT_TM,

    c.UnloadedLotNo,
    c.WbEmptyWt as Rec_WbEmptyWt, c.WbLoadWt as Rec_WbLoadWt, c.WbNetWt as Rec_WbNetWt, 
    c.GunnyWt as Rec_GunnyWt, c.GunnyLessNetWt as Rec_GunnyLessNetWt,c.UnLoadingVendor,c.UnloadingChargePerTon,

    (a.GunnyLessNetWt - c.GunnyLessNetWt) as TotalMovementQty,
    '".$StartDate."' as start_Dt , '".$EndDate."' as end_Dt,

    pi.GateInBy, UI1.FIRST_NAME as GateInByName, pi.GateOutByName, pi.QualityCheckSubmitByName, 
    pi.FirstWeightEntryByName, pi.SecondWeightEntryByName,

    DATE_FORMAT(pi.GateInDt, '%d-%m-%Y %H:%i') RecGateInDt, 
    DATE_FORMAT(pi.QualityCheckSubmitDt, '%d-%m-%Y %H:%i') RecQualityCheckSubmitDt, 
    DATE_FORMAT(pi.FirstWeightEntryDt, '%d-%m-%Y %H:%i') RecFirstWeightEntryDt, 
    DATE_FORMAT(pi.SecondWeightEntryDt, '%d-%m-%Y %H:%i') RecSecondWeightEntryDt, 
    DATE_FORMAT(pi.GateOutDt, '%d-%m-%Y %H:%i') RecGateOutDt

FROM intrastate_warhouse_dispatch_info a
JOIN empty_vehicle_arrival b 
    ON a.VehicleArrivalId = b.ID
LEFT JOIN intrastate_gateout_info c 
    ON a.VehicleArrivalId = c.EmptyVehicleArrivalId 
   AND c.ReceivingArrivalId IS NOT NULL
LEFT JOIN purchase_info pi 
    ON a.VehicleArrivalId = pi.EMPTY_VEHICLE_ARRIVAL_ID
LEFT JOIN user_info UI1 
    ON pi.GateInBy = UI1.UI_ID
LEFT JOIN user_info UI2 
    ON b.GateInBy = UI2.UI_ID
WHERE 1 $cnd";
    ...
    public function getLotByPlant( $PlantId){
      ...
      public function saveGateOutInfo($postData){
        ...
        $builder = $this->db->query($sql);
/* Mohan commented on 14092022 for checking duplicate actual insert
