...
class STOPODeliveryPlanModel extends Model
{
  ...
  public function getLastTicNo($plant)
  {
    $builder = $this->db->table("ngw_weeklyplan");
    $builder = $builder->select("ngw_weeklyplan.planid,ngw_weeklyplan.transcation_unique_no");
    $builder = $builder->where('ngw_weeklyplan.fromplantid', $plant);
    $builder = $builder->orderBy('ngw_weeklyplan.planid', 'DESC')
      ->limit(1)
      ->get()
      ->getResultArray();
  ...
  public function  getcountstatus($plant)
  {
    $builder1 = $this->db->table("delivery_control_panel");
		...
		$builder1 = $builder1->where("delivery_control_id",8);
    ...
    //print_r($days_sdi);exit;
    $date_sdi  = date('Y-m-d', strtotime("$days_sdi day", strtotime($EndDate)));
    $builder = $this->db->table("ngw_weeklyplan");
    $builder = $builder->select("ngw_weeklyplan.*");
    $builder = $builder->where("ngw_weeklyplan.Status", 1);
    $builder = $builder->where("ngw_weeklyplan.MovementType = 'GODOWN'");
    $builder = $builder->where('ngw_weeklyplan.fromplantid', $plant);
    $builder = $builder->where("ngw_weeklyplan.InsDt<=",$date_sdi);
  ...
  public function  getcountduplicate($fromlotnum,$wheatvariety)
  {
    $builder = $this->db->table("ngw_weeklyplan");
    $builder = $builder->select("ngw_weeklyplan.*");
    $builder = $builder->where("ngw_weeklyplan.Status", 1);
    $builder = $builder->where("ngw_weeklyplan.MovementType = 'GODOWN'");
    $builder = $builder->where('ngw_weeklyplan.fromlotno', $fromlotnum); 
    $builder = $builder->where('ngw_weeklyplan.wheatvarityid', $wheatvariety);
    $count = $builder->countAllResults();
    //$count=0;
    return $count; // Return the count of records
  }
  ...
  public function getGodowntoGodown($postData)
  {
    ...
    if ($postData->Screen =="GTG_EDIT"){
      ...
      $Cnd = " AND DATE_FORMAT(a.InsDt, '%Y-%m-%d') >= '$fDate'
                    AND DATE_FORMAT(a.InsDt, '%Y-%m-%d') <= '$tDate' ";
    }

    $fetchsql = "SELECT a.planid,a.transcation_unique_no,
                  ...
                  WHERE a.RecStatus='1'
                    AND a.MovementType='GODOWN'
                    $Cnd";
    $builder = $this->db->query($fetchsql);
    $bulibder = $builder->getResultArray();
    // print_r($fetchsql);
    // exit;
    return $builder->getResultArray();
  }
...
//var_dump($Data);exit();
    ...
    public function getWarehouse($MONTH){
      $fetchsql = "SELECT WH_REFID as value, WH_NAME as label
      FROM `warehouse_master` a
      JOIN ngw_weeklyplan b ON a.WH_REFID=b.fromwarehouseid
      where b.PlanMonth='".$MONTH."' AND b.towarehouseid = 0 GROUP BY a.WH_REFID";    
      $builder = $this->db->query($fetchsql);
      return  $builder->getResultArray();
    }

    public function getPlant($MONTH,$WAREHOUSE_ID){
      $fetchsql = "SELECT ID as value, PLANT_NAME as label,plant_subdivision
      FROM `master_plant` a
      JOIN ngw_weeklyplan b ON a.ID=b.fromplantid
      where b.PlanMonth='".$MONTH."' AND b.fromwarehouseid='".$WAREHOUSE_ID."' AND b.towarehouseid = 0 GROUP BY a.ID";    
      $builder = $this->db->query($fetchsql);
      return  $builder->getResultArray();
    }

    public function getWheat($MONTH,$WAREHOUSE_ID,$PLANT_ID){
      $fetchsql = "SELECT Id as value, WheatVariety as label
      FROM `master_mrc_wheat_variety` a
      JOIN ngw_weeklyplan b ON a.Id=b.wheatvarityid
      where b.PlanMonth='".$MONTH."' AND b.fromwarehouseid='".$WAREHOUSE_ID."' AND b.fromplantid='".$PLANT_ID."' AND b.towarehouseid = 0 GROUP BY a.Id";    
      $builder = $this->db->query($fetchsql);
      return  $builder->getResultArray();
    }

    public function getPlanDetails($MONTH,$WAREHOUSE_ID,$PLANT_ID,$WHEAT_ID){
      $fetchsql = "SELECT a.planid,a.planqty,b.Segment,b.Id,b.WheatVariety,c.WERKS
      FROM `ngw_weeklyplan` a
      JOIN master_mrc_wheat_variety b ON a.wheatvarityid=b.Id
      JOIN master_plant c ON $PLANT_ID=c.ID
      where a.PlanMonth='".$MONTH."' AND a.fromwarehouseid='".$WAREHOUSE_ID."' AND a.fromplantid='".$PLANT_ID."' AND a.wheatvarityid='".$WHEAT_ID."' AND a.towarehouseid = 0 ORDER by a.planid";    
      $builder = $this->db->query($fetchsql);
      return  $builder->getResultArray();
    }

    public function getVendorDetails(){
      $fetchsql = "SELECT Id as value, Name as label
      FROM `master_vendor`
      where Category = 'Last Mile Transporter' or 'LAST MILE TRANSPORTER' AND RecStatus = 1 ORDER by Id";    
      $builder = $this->db->query($fetchsql);
      return  $builder->getResultArray();
    }
    ...
    public function getVendorDetailsLoading(){
      $fetchsql = "SELECT Id as value, Name as label
      FROM `master_vendor`
      where Category = 'Loading Vendor' or 'LOADING VENDOR' AND RecStatus = 1 ORDER by Id";    
      $builder = $this->db->query($fetchsql);
      return  $builder->getResultArray();
    }
    public function getVendorDetailsUnloading(){
      $fetchsql = "SELECT Id as value, Name as label
      FROM `master_vendor`
      where Category = 'Unloading Vendor' or 'UNLOADING VENDOR' AND RecStatus = 1 ORDER by Id";    
      $builder = $this->db->query($fetchsql);
      return  $builder->getResultArray();
    }
    public function getVendorByID($Vendor_ID){
      $fetchsql = "SELECT Id,Code
      FROM `master_vendor`
      where Id = '$Vendor_ID'";    
      $builder = $this->db->query($fetchsql);
      return  $builder->getResultArray();
    }
    public function getStorageLocationListToPlant($Id){
      $fetchsql = "SELECT a.STORAGE_REFID as value,a.STORAGE_LOCATION as label,b.WERKS
      FROM master_storage a,master_plant b where a.plantid='$Id' and b.ID='$Id' and a.RecStatus='1'";
      $builder =  $this->db->query($fetchsql);
      return  $builder->getResultArray();
    }
    public function Insert_sto_po($Data){
      $this->db->table('plan_sto_po')->set($Data)->insert();
      $InsId=$this->insertID();
      return $InsId;
    }
    public function ngw_weeklyplan_update($Data,$planid,$to_warehouse_id){
      $updateSql = "UPDATE ngw_weeklyplan SET towarehouseid = $to_warehouse_id,toplantid = $Data->toplantid,tolocationid = $Data->tolocationid WHERE planid in ('$planid')";
      $this->db->query($updateSql);
      $InsId=$planid; 
      return $InsId;
    }
    public function  PlanSTOPODataGet($status){


      $fetchsql = "SELECT a.*,b.WH_NAME as fromwarehouse,c.WheatVariety,e.PLANT_NAME as fromplant,f.STORAGE_LOCATION,g.Name as freightvendor,h.Name as loadingvendor,i.Name as unloadingvendor,d.PLANT_NAME as toplantname,j.WH_NAME as towarehouse,
      IF(a.status=1,'Created',IF(a.Status=2,'Purchase Approved',IF(a.Status=3,'QA Approved','Rejected'))) as StatusName
      FROM `plan_sto_po` a
      LEFT JOIN warehouse_master b ON a.from_warehouse_id=b.WH_REFID
      LEFT JOIN master_mrc_wheat_variety c ON a.wheat_variety_id=c.Id
      LEFT JOIN master_plant d ON a.to_plant_id=d.ID
      LEFT JOIN master_plant e ON a.from_plant_id=e.ID
      LEFT JOIN master_storage f ON a.to_storage_id=f.STORAGE_REFID
      LEFT JOIN master_vendor g ON a.freight_vendor_id=g.Id
      LEFT JOIN master_vendor h ON a.loading_vendor_id=h.Id
      LEFT JOIN master_vendor i ON a.unloading_vendor_id=i.Id
      LEFT JOIN warehouse_master j ON a.to_warehouse_id=j.WH_REFID
      where a.status in ($status) ORDER BY a.id";
    //  echo $fetchsql;exit;
      $builder =  $this->db->query($fetchsql);
      return  $builder->getResultArray();
    }

    public function  PlanSTOPODataReport($status,$fromdate,$todate){
    ...
    //  echo $fetchsql;exit;
     ...
     public function  STOPODeliveryPlanByID($id){

      $fetchsql = "SELECT a.*,b.WH_NAME as fromwarehouse,c.WheatVariety,c.Segment,e.PLANT_NAME as fromplant,f.STORAGE_LOCATION,g.Name as freightvendor,h.Name as loadingvendor,i.Name as unloadingvendor,d.PLANT_NAME as toplantname,j.WH_NAME as towarehouse,g.Code as freightvendorcode,h.Code as loadingvendorcode,i.Code as unloadingvendorcode,e.WERKS as fromplantcode,d.WERKS as toplantcode,b.WH_CODE as fromwarehousecode,j.WH_CODE as towarehousecode,c.MaterialCode,
      date_format(a.qa_approved_at,'%Y-%m-%d') as qa_approved_date,
      date_format(a.purchase_approved_at,'%Y-%m-%d') as purchase_approved_date,f.LGORT
    ...
    public function ngw_weeklyplan_reject($planid){
      $updateSql = "UPDATE ngw_weeklyplan SET towarehouseid = 0,toplantid = 0,tolocationid = 0 WHERE planid in ($planid)";
      $this->db->query($updateSql);
      $InsId=$planid; 
      return $InsId;
    }
    public function Update_sto_po($Data,$id){

      $InsId = $this->db->table('plan_sto_po')->set($Data)->where('id',$id)->update();
      return $id;
    }

    public function getWHplantList($WH_CODE,$PLANT_ID)
  {
    $Sql = "SELECT plant_subdivision
            FROM  master_plant  
            WHERE ID = '".$PLANT_ID."' ";
    $builder = $this->db->query($Sql);
    $plant_data = $builder->getResultArray();

    $Sql1 = "SELECT ID as value,WERKS as label
            FROM  master_plant  
            WHERE WH_CODE = '".$WH_CODE."' and plant_subdivision = '".$plant_data[0]['plant_subdivision']."'";
    // print_r($Sql1);exit;
    $builder1 = $this->db->query($Sql1);
    return  $builder1->getResultArray();
  }
}
