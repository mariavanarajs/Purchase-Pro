...
function fetchPODetailsById($connect, $record)
{
  ...
  $fetchsql =
    "SELECT stp.refid, EBELN, EBELP, BROCKER_CODE, BROCKER_NAME, MENGE, MEINS, IDNLF, MATNR, SGT_SCAT, NETPR, PLANT_NAME, STORAGE_LOCATION, LOEKZ, INCO1, INCO_DESC, BSART,Loading_cost,Unloading_cost,Freight_cost,DATE_FORMAT(PO_LOADING_DATE,'%d-%m-%Y') as POLOADINGDATE, NUMBER_OF_VEHICLES, PURCHASE_ORG, PURCHASE_ORG_DESC as VEHICLETYPE, BAG_NAME as PO_Bag_Type, mpi.WERKS as plant_id ,msi.LGORT as storage_id, msi.plantid as toplantid, msi.STORAGE_REFID as tolocationid
    FROM
    sap_to_pp stp 
