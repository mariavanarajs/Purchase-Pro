...
if ($entityVAContent->formType == "getRelottingforApproval" || $entityVAContent->formType == "getRelotting" || $entityVAContent->formType == "getRelottingforEntry" || $entityVAContent->formType == "getRelottingProcessChange" || $entityVAContent->formType == "getRelottingProcessChangeWHMG" || $entityVAContent->formType == "getRelottingProcessChangeWHACCMG" ) 
{
...
if($record->fromwarehouseid != '') {
    ...
    $sdt = "select id,temp_extended_days FROM delivery_control_panel where status = 2 and delivery_control_id = 5 order by id";
    $res = getResultAsObjectArray($connect, $sdt);
    $temp_extended_days = $res[0]["temp_extended_days"];
    $days_sdi = -2 - $temp_extended_days ;
    $date_sdi  = date('Y-m-d', strtotime("$days_sdi day", strtotime($EndDate)));
    // print_r($date_sdi);exit();

    $ngw_relots = "select fromwarehouseid from ngw_relot where fromwarehouseid='$record->fromwarehouseid' and RelotDate <= '".$date_sdi."'
  and RelotStatus not in (4,-1,7)";
    $res = getResultAsObjectArray($connect, $ngw_relots);
...
}if($record->formType == "getRelottingProcessChange" )
{
  array_push($filters, " RelotStatus IN('1','2','3','-1') ");
}if($record->formType == "getRelottingProcessChangeWHMG" )
{
  array_push($filters, " RelotStatus IN('5') ");
}if($record->formType == "getRelottingProcessChangeWHACCMG" )
{
  array_push($filters, " RelotStatus IN('6') ");
}
...
IF(a.RelotStatus='5','WH Incharge Reject',
IF(a.RelotStatus='6','WH MG Reject',
