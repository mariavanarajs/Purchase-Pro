...
if ($entityVAContent->formType == "getRelottingforApproval" || $entityVAContent->formType == "getRelotting" || $entityVAContent->formType == "getRelottingforEntry" ) {
 ...
 echo PhysicalstockEntry($connect, $entityVAContent);
}

function PhysicalstockEntry($connect,$record){
  //var_dump($record);exit();
    //Check Duplicate
    /*$sql = "SELECT * FROM `ngw_physical_stock` where lotid='".$record->lotid."' 
 ...
 // array_push($values, mysqli_real_escape_string($connect, trim($SessionUser)));
  ...
  $sql = "INSERT INTO ngw_physical_stock (" . join(", ", $fields) . ") VALUES ( '" . join("','", $values) . "')";
  //echo $sql;exit();
  mysqli_query($connect, $sql);
  $last_id = $connect->insert_id;

  $sql = "UPDATE `ngw_sublot` SET `Last_Physical_Stock_Id`='$last_id' 
  where `warehouseid`='".$record->warehouseid."' and `plantid`='".$record->plantid."' and 
  `wheatvarietyid`='".$record->Wheat_Variety_Id."' and lotid='".$record->lotid."' and `RecStatus`='1'";
  //echo $sql;
  mysqli_query($connect, $sql);
...
where  ".join(' AND ', $filters)." order by RelotId ";
//echo $fetchsql;
