...
switch ($formType) {
  ...
  echo Get_Control_list_IAS_STM($connect,$record);
        break; 
        //courier
        case "GetCourierMaster":
          echo GetCourierMaster($connect,$record);
                break;
...
{
    /* ===============================
       STEP 1 – FILTER & PAGINATION
    =============================== */

    $filters = ["a.USER_STATUS < '2'", "a.RecStatus = '1'"];

    if (!empty($record->searchTxt)) {
        $searchTxt = $record->searchTxt;
        $filters[] = "(
            a.FIRST_NAME LIKE '%$searchTxt%' OR
            a.LOGIN_ID LIKE '%$searchTxt%' OR
            a.SI_NO LIKE '%$searchTxt%' OR
            e.EMP_NAME LIKE '%$searchTxt%' OR
            e.EMP_CODE LIKE '%$searchTxt%'
        )";
    }
    ...
    $limit = "";
    if (isset($record->startCount) && isset($record->pageSize)) {
        $limit = " LIMIT {$record->startCount}, {$record->pageSize}";
    }

    /* ===============================
       STEP 2 – FETCH USERS ONLY
    =============================== */

    $userSql = "
        SELECT 
            ROW_NUMBER() OVER (ORDER BY a.UI_ID ASC) AS S_NO, a.UI_ID, a.FIRST_NAME, a.LOGIN_ID, a.SI_NO, a.DESIGNATION, c.ROLE_NAME, a.DEPARTMENT, a.CITY, a.STATE, a.USER_ROLE_ID, a.USER_STATUS, b.label AS ACTIVELABEL, a.InsBy, a.InsDt, a.ModBy, a.ModDt, a.RecStatus, a.LoginStatus, a.LoginTime, a.LogoutTime, CASE WHEN a.USER_ROLE_ID IN (5,12,13,14,15) THEN c.ROLE_NAME ELSE e.emp_department END AS emp_department, CASE WHEN a.USER_ROLE_ID IN (5,12,13,14,15) THEN c.ROLE_NAME ELSE e.emp_designation END AS emp_designation, CASE WHEN a.USER_ROLE_ID IN (5,12,13,14,15) THEN c.ROLE_NAME ELSE e.emp_division END AS emp_division,m.gateName
        FROM user_info a
        LEFT JOIN employee_master e ON e.emp_code = a.EMP_CODE
        LEFT JOIN master_gate m ON m.id = a.masterGateId
        INNER JOIN master_role c ON c.RM_REFID = a.USER_ROLE_ID
        INNER JOIN activestatus b ON b.ListGroup = 'G2' AND b.id = a.USER_STATUS
        WHERE $where
        ORDER BY a.UI_ID ASC
    ";
    // print_r($userSql); exit;
    $users = getResultAsObjectArray($connect, $userSql);

    if (empty($users)) {
        return json_encode(["success" => 1, "results" => [], "count" => 0]);
    }
    ...
    $userIds = array_column($users, 'UI_ID');
    ...
    // Plant Access
    $plantSql = "
        SELECT up.USER_ID, GROUP_CONCAT(DISTINCT mp.WERKS) AS plant_access
        FROM master_user_plant up
        LEFT JOIN master_plant mp ON mp.ID = up.PLANT_ID
        WHERE up.USER_ID IN ($idList) AND up.RecStatus='1'AND mp.plant_subdivision IN (0,1)
        GROUP BY up.USER_ID
    ";
    $plants = getResultAsObjectArray($connect, $plantSql);
    
    // Screen Access
    $screenSql = "
        SELECT us.USER_ID, GROUP_CONCAT(DISTINCT ms.SCREEN_DESC) AS screen_access
        FROM master_user_screen us
        LEFT JOIN master_screen ms ON ms.ID = us.SCREEN_ID
        WHERE us.USER_ID IN ($idList) AND us.RecStatus='1'
        GROUP BY us.USER_ID
    ";
    $screens = getResultAsObjectArray($connect, $screenSql);
 
    // Module Access
    $moduleSql = "
        SELECT um.userInfoId AS USER_ID, 
               GROUP_CONCAT(DISTINCT mm.moduleType) AS module_access
        FROM user_module_access um
        LEFT JOIN master_module mm ON mm.id = um.moduleTypeId
        WHERE um.userInfoId IN ($idList) AND um.isAccess='1'
        GROUP BY um.userInfoId
    ";
    $modules = getResultAsObjectArray($connect, $moduleSql);

    // Privilege Access
    $privSql = "
        SELECT mup.USER_ID, 
               GROUP_CONCAT(DISTINCT mp.PRIVILEGE_DESC) AS privilege_access
        FROM master_user_privilege mup
        LEFT JOIN master_privilege mp ON mp.ID = mup.PRIVILEGE_ID
        WHERE mup.USER_ID IN ($idList)
        GROUP BY mup.USER_ID
    ";
    $privileges = getResultAsObjectArray($connect, $privSql);

    // PO Type Access
    $poSql = "
        SELECT pt.userInfoId AS USER_ID,
               GROUP_CONCAT(DISTINCT p.name) AS po_type_access
        FROM po_type_access pt
        LEFT JOIN po_type p ON p.id = pt.poTypeId
        WHERE pt.userInfoId IN ($idList)
        GROUP BY pt.userInfoId
    ";
    $poTypes = getResultAsObjectArray($connect, $poSql);
   ...
   function toMap($array, $key, $value)
    ...
    $plantMap = toMap($plants, 'USER_ID', 'plant_access');
    $screenMap = toMap($screens, 'USER_ID', 'screen_access');
    $moduleMap = toMap($modules, 'USER_ID', 'module_access');
    $privMap = toMap($privileges, 'USER_ID', 'privilege_access');
    $poMap = toMap($poTypes, 'USER_ID', 'po_type_access');
    
    /* ===============================
       STEP 6 – MERGE DATA
    =============================== */
    $user = '';
    // print_r($plantMap); exit;
    foreach ($users as &$user)
    {
        $uid = $user['UI_ID'];

        $user['plant_access']     = $plantMap[$uid] ?? "";
        $user['screen_access']    = $screenMap[$uid] ?? "";
        $user['module_access']    = $moduleMap[$uid] ?? "";
        $user['privilege_access'] = $privMap[$uid] ?? "";
        $user['po_type_access']   = $poMap[$uid] ?? "";
    }
...
function GetMaster_user_screenlist($connect,$record){
  $filters=array("a.RecStatus in ('0','1')");
  if (isset($record->searchTxt)) {
...
//Courier 
function GetCourierMaster($connect,$record){
  
  $filters=array("Status='1'");
  if (isset($record->searchTxt)) {
    array_push($filters, "(courier_name like '%" . $record->searchTxt . "%'  
    or id like '%" . $record->searchTxt . "%'
    )");
  }
  $fetchsql = "SELECT `id`, `courier_name`, `status`, `created_by`, `created_at`, `updated_by`, `updated_at` 
  FROM `cuorier_master` WHERE ". join(" AND ", $filters). "ORDER by Id DESC";
  $record->startCount . ",".$record->pageSize;
  $tableRecords = getResultAsObjectArray($connect, $fetchsql);
 // return json_encode(["success" => 1, "results" => $tableRecords]);
  $countsql = "SELECT count(1) as total FROM `cuorier_master` WHERE `Status`='1'"; 
  $total = getFieldValue($connect, $countsql,0);
  return json_encode(["success" => 1, "results" => $tableRecords, "count" => $total]);
}
