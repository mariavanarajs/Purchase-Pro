  ...
  while (false !== ($SAP_DATA = readdir($handle))) {
    if ($SAP_DATA != "." && $SAP_DATA != "..") {
      if ($SAP_DATA != "archive") {
         ...
         if ($fileexists == 1) {
          ...
          foreach ($importData_arr as $data) {
            if ($skip != 0) {
              ...
              $HLWeight = mysqli_real_escape_string($connect, trim($data[17]));
              $ForesightMatter = mysqli_real_escape_string($connect, trim($data[18]));
              $bad_smell = mysqli_real_escape_string($connect, trim($data[19]));
              $Dust = mysqli_real_escape_string($connect, trim($data[20]));
              $seivesize = mysqli_real_escape_string($connect, trim($data[21]));
              

              if ($ProductName == 'Wheat Naga') {
              ...
              `ForeignMatter`, `BadSmell`,`Dust`, `SeiveSize`,  `VANumber`)
                 ...
                 '$SV','$Infestation','$TW','$ForesightMatter','$bad_smell','$Dust','$seivesize','$VANumber')";
       ...
       var_dump($new_target_location);
        if ($ProductName == 'Wheat Naga') {
        if (rename($target_file, $new_target_location))
        //print_r($new_target_location);exit;
          echo 'done';
       // unlink($target_file);
        else
          echo 'not done';
