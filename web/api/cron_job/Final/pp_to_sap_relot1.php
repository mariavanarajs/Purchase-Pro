...
{
  $cnd = " and RelotStatus in (2,3,4) ";
}


$output_csv = "SELECT * FROM view_ngw_relot WHERE SAPStatus = 0 and RelotStatus in (3,4)$cnd";
//echo $output_csv;
$response="RESPONSE";
...
if (mysqli_num_rows($result) > 0) {
    ...
    // $rowArr[]=$output;
     while ($row = mysqli_fetch_array($result)) {
            ...
            "wh_code"=>trim($row['wh_code']), 
            "werks"=>trim($row['from_werks']),
            "LGORT"=>trim($row['FROM_LGORT']),
             "fromlotno"=>trim($row['fromlotno']), 
            ...
            "GunnylessWeight"=>trim($row['GunnylessWeight']),
            ...
            "FreightVendorCode" =>trim($row['FreightVendorCode']),
