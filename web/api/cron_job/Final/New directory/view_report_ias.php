...
if (mysqli_num_rows($result) > 0) {
 $fp = fopen('/home/purpro/SAP/PP/ias/pp_sap_ias/' . $date . 'view_report_ias.csv', 'w');
   ...
   // $fp = fopen($fname, 'w');
    ...
    'QI_REFID',    'purchase_info_id',     'moisture_quality',       'hl_quality',   'infestation_quality', 'Receiving Bag Type','No Of Bags' , 'Receiving Bag Type(2)','No Of Bags(2)' , 'Receiving Bag Type(3)','No Of Bags(3)'  );
...
//exit();
       ...
       trim($row['QI_REFID']),    trim($row['purchase_info_id']),    trim($row['moisture_quality']),     trim($row['hl_quality']),    trim($row['infestation_quality']), trim($row['rBagType']), trim($row['rno_bags']) ,trim($row['rBagType2']) ,trim($row['rno_bags2']) ,trim($row['rBagType3']) ,trim($row['rno_bags3'])  );
       fputcsv($fp, $test, ",", " ");
