...
$date = date("YmdHis");
$output_csv = "SELECT * FROM view_ngw_relot WHERE isSAPUpdated = 0 ";
// echo $output_csv;
$response="RESPONSE";
...
// $output = array('wirqstdate', 'RelotDate', 'WheatVarietyId', 'ngw_materialcode.materialcode', 'wh_refid', 'WERKS', 'LGORT', 'lotno', 'lotno', 'bag_type', 'NoOfBags', 'VERME', 'Code', 'RelottingCharges', 'Relotreason', 'RelotStatus');
  ...
  $Qry="UPDATE `pp_lastsynclog` SET  LastRunDate='$CurrentDateTime' where UniqueName='pp_to_sap_RelotEntrylastsyncdate";
  mysqli_query($connect, $Qry);
  SendToSap($rowArr);
  //echo json_encode(array($response=>'success',"Relot_Entry"=>$rowArr));
}
else
{
