...
if (mysqli_num_rows($result) > 0) {
    $fp = fopen('/home/purpro/SAP/PP/silo_to_mill/pp_sap_delivery_data/' . $date . 'pp_sap_delivery_data.csv', 'w');
...
//$fp = fopen($fname, 'w');
   ...
   $output = array('ZVA_NUMBER', 'DRIVER_NO',    'TRUCK_NO',    'PLANT_NAME', 'STORAGE LOCATION',   'ZPO_NUMBER',    'PO_LINE_ITEM','WheatVariety','ReceivingBin','BulkSiloNo','Moisture','HLWeight','FM','Dust','BadSmell','Infestation','SeiveSize','Wetgluten','Drygluten','Protein','FirstWeight','SecondWeight','NetWeight','GateIN','GateOut');
    fputcsv($fp, $output);
    while ($row = mysqli_fetch_array($result)) {
        $ID = $row['ID'];
      $test = array(trim($row['ZVA_NUMBER']),    trim($row['DRIVER_NO']),    trim($row['TRUCK_NO']),    trim($row['PLANT_NAME']),  trim($row['StorageLocation']),    trim($row['ZPO_NUMBER']),    trim($row['PO_LINE_ITEM']),    trim($row['WheatVariety']),    trim($row['ReceivingBin']),    trim($row['BulkSiloNo']),    trim($row['Moisture']),    trim($row['HLWeight']),    trim($row['FM']),    trim($row['Dust']),    trim($row['BadSmell']),    trim($row['Infestation']),    trim($row['SeiveSize']),    trim($row['Wetgluten']),    trim($row['Drygluten']),    trim($row['Protein']),    trim($row['FirstWeight']),    trim($row['SecondWeight']),    trim($row['NetWeight']),trim($row['GateIn']),trim($row['GateOutDt']));
