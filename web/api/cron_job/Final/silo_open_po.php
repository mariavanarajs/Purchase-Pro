...
//$date = date("YmdHis");
if ($handle = opendir("/var/www/purchaseprouat/sapfileshare/silo_to_mill/openstopo")) {

    while (false !== ($SAP_DATA = readdir($handle))) {
        if ($SAP_DATA != "." && $SAP_DATA != "..") {
            if ($SAP_DATA != "archive") {
                $target_file = '/opt/lampp/htdocs/SAP/silo_to_mill/openstopo/' . $SAP_DATA;
                $new_target_location = "/opt/lampp/htdocs/SAP/silo_to_mill/openstopo/archive/" . $SAP_DATA;
