...
if ($handle = opendir("/var/www/purchaseprouat/sapfileshare/openpos")) {
  while (false !== ($SAP_DATA = readdir($handle))) {
    if ($SAP_DATA != "." && $SAP_DATA != "..") {
      if ($SAP_DATA != "archive") {
        $target_file = "/var/www/purchaseprouat/sapfileshare/openpos/" . $SAP_DATA;
        $new_target_location = "/var/www/purchaseprouat/sapfileshare/openpos/archive/" . $SAP_DATA;
        //echo $target_file;
        $fileexists = 0;
        ...
        if ($fileexists == 1) {
          ...
          echo "<br>new_target_location->$new_target_location";
          ...
          // $rename_test = rename($target_file, $new_target_location);
          // if($rename_test){
          //   echo "success";
          // } else {
          //   echo "Failure";
