...
eva.WB_SERIAL_NO, eva.WB_EMPTY_WT, eva.TRUCK_NO, eva.WB_TICKET_NO, eva.WB_CHARGES, eva.SCREEN_TYPE, eva.VEHICLE_STATUS,
 ...
 qi.moisture_quality as ZMOISTURE_QUALITY1,qi.hl_quality as ZHL_QUALITY1,qi.insect_damage_wheat_quality as ZINSECT_DAMAGE_WHEAT_QUALITY1
 ...
 JOIN purchase_info pi ON pi.EMPTY_VEHICLE_ARRIVAL_ID = intrad.VehicleArrivalId and pi.ZVA_NUMBER=eva.ZVA_NUMBER and pi.VECHICAL_STATUS=12
 LEFT JOIN quality_info qi ON pi.PI_REFID=qi.purchase_info_id
  where eva.IsUpdated = '1'  $cnd";
 //echo $query;exit();
/*
...
if (mysqli_num_rows($result) > 0) {
    ...
    // $rowArr[]=$output;
     while ($row = mysqli_fetch_assoc($result)) {
        ...
        $update = "UPDATE empty_vehicle_arrival SET IsUpdated = '2' WHERE Id = ".$row['VehicleArrivalId']."";
        // echo $update;
  ...
  $Qry="UPDATE `pp_lastsynclog` SET  LastRunDate='$CurrentDateTime' where UniqueName='pp_to_sap_ias_rgo'";
  mysqli_query($connect, $Qry);
  SendToSap($rowArr);
  echo json_encode(array($response=>'success',"pp_to_sap_ias_rgo"=>$rowArr));
  
}
else
{
    echo json_encode(array($response=>'ERROR',"Message"=>"NO RECORDS FOUND", "pp_to_sap_ias_rgo"=>[]));
}
...
curl_setopt_array($curl, array(
  ...
  CURLOPT_URL => ''.$SAP_HOSTNAME.'zwh_pp_sap/ias_data?sap-client=900',
...
function GetSAP_CSRF_Token()
{
  ...
  //exit();
  // exit();
  $TmpUrl = ''.$SAP_HOSTNAME.'zwh_pp_sap/ias_data?sap-client=900';
  //echo "<br>".$TmpUrl."<br>";
$curl = curl_init();
curl_setopt_array($curl, array(
  //CURLOPT_URL => 'http://nfapplqua1.nagaltd.com:8001/zcomplaint/webcomplaint?sap-client=900',
