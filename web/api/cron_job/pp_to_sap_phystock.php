...
$SAP_HOSTNAME = "10.10.63.134";//SAP HOST NAME nfapplqua1.nagaltd.com
...
$date = date("YmdHis");
$output_csv = "SELECT * FROM view_ngw_physicalinventory WHERE isSAPUpdated = 0 and screentype = 'WHEAT MOVEMENT'";
//echo $output_csv;
$response="RESPONSE";
...
if (mysqli_num_rows($result) > 0) {
    ...
    // $rowArr[]=$output;
     while ($row = mysqli_fetch_array($result)) {
            $test = array("Physical_Inventory_Id"=>trim($row['Physical_Inventory_Id']), 
            "Posting_Date"=>trim($row['Posting_Date']), 
            "lotno"=>trim($row['lotno']), "werks"=>trim($row['werks']), 
            "warehouseid"=>trim($row['warehouseid']), 
            "StorageLocationId"=>trim($row['StorageLocationId']), 
            "Wheat_Variety_Id"=>trim($row['Wheat_Variety_Id']), 
            "MaterialCode"=>trim($row['MaterialCode']), 
            "Physical_Qty"=>trim($row['Physical_Qty']), 
            "SAP_Qty"=>trim($row['SAP_Qty']),
            "UP_Down_Qty"=>trim($row['UP_Down_Qty']),
            "Screentype"=>trim($row['Screentype']));
        $rowArr[]=$test;
        $update = "UPDATE ngw_physicalinventory SET isSAPupdated = '1' WHERE Physical_Inventory_Id = ".$row['Physical_Inventory_Id'];
...
function GetSAP_CSRF_Token()
{
  ...
  //exit();
  // exit();
  $TmpUrl = 'http://'.$SAP_HOSTNAME.':8001/zcomplaint/webcomplaint?sap-client=900';
  //echo "<br>".$TmpUrl."<br>";
$curl = curl_init();
curl_setopt_array($curl, array(
  //CURLOPT_URL => 'http://nfapplqua1.nagaltd.com:8001/zcomplaint/webcomplaint?sap-client=900',
