...
// function getIASPOEditDet($PoNo, $VA_No){
...
eva.WB_SERIAL_NO, eva.WB_EMPTY_WT, eva.TRUCK_NO, eva.WB_TICKET_NO, eva.WB_CHARGES, eva.SCREEN_TYPE, eva.VEHICLE_STATUS,
 ...
 eva.tripsheet_no, eva.driver_name
 ...
 WHERE eva.IsUpdated = 1 and eva.Reversal_Flag='COMPLETED' $cnd";
// echo $query;
...
if (mysqli_num_rows($result) > 0) {
    ...
    // $rowArr[]=$output;
     while ($row = mysqli_fetch_array($result)) {
        ...
        $update = "UPDATE empty_vehicle_arrival SET isUpdated = '1', Reversal_Flag='SAP UPDATED' WHERE Id = ".$row['VehicleArrivalId']."";
        // echo $update;
  ...
  $Qry="UPDATE `pp_lastsynclog` SET  LastRunDate='$CurrentDateTime' where UniqueName='pp_to_sap_ias_reversal'";
  mysqli_query($connect, $Qry);
  ////SendToSap($rowArr);
  echo json_encode(array($response=>'success',"pp_to_sap_ias_reversal"=>$rowArr));
  
}
else
{
    echo json_encode(array($response=>'ERROR',"Message"=>"NO RECORDS FOUND", "pp_to_sap_ias_reversal"=>[]));
}
