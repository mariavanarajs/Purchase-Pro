...
//$date = date("YmdHis");
$URL="E:\\Foresight\\PurchasePro\\api\\cron_job\\Foresight\\IAS\\";
         ...
         if ($fileexists == 1) {
          ...
          foreach ($importData_arr as $data) {
            if ($skip != 0) {
              ...
              $HL = mysqli_real_escape_string($connect, trim($data[15]));
              $InsertDamageWheat = mysqli_real_escape_string($connect, trim($data[16]));
              $VANumber = mysqli_real_escape_string($connect, trim($data[17]));
              $insert_query = "INSERT INTO `ias_qcdetails`(`AnalysisTime`, `ProductName`, `ProductCode`, 
              `SampleType`, `SampleNumber`, `SampleComment`, `InstrumentName`, `InstrumentSerialNumber`, `Protein`,
               `Moisture`, `Ash`, `Wetgludten`, `Drygluten`, `SV`, `Infestation`, `HL`, `InsertDamageWheat`,
               `VANumber`) VALUES ('$AnalysisTime','$ProductName','$ProductCode','$SampleType','$SampleNumber',
               '$SampleComment','$InstrumentName','$InstrumentSerialNumber','$Protein','$Moisture','$Ash',
               '$Wetgludten','$Drygluten','$SV','$Infestation','$HL','$InsertDamageWheat','$VANumber')";
