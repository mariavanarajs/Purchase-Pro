...
//$date = date("YmdHis");
$URL="E:\\Foresight\\PurchasePro\\api\\cron_job\\Foresight\\STM\\";
         ...
         if ($fileexists == 1) {
          ...
          foreach ($importData_arr as $data) {
            if ($skip != 0) {
              ...
              $Infestation = mysqli_real_escape_string($connect, trim($data[14]));
              $HLWeight = mysqli_real_escape_string($connect, trim($data[15]));
              $ForesightMatter = mysqli_real_escape_string($connect, trim($data[16]));
              $bad_smell = mysqli_real_escape_string($connect, trim($data[17]));
              $Dust = mysqli_real_escape_string($connect, trim($data[18]));
              $seivesize = mysqli_real_escape_string($connect, trim($data[19]));
              $VANumber = mysqli_real_escape_string($connect, trim($data[20]));

               
            
              $insert_query = "INSERT INTO `stm_silotomillqcdet`( AnalysisTime,`ProductName`, `ProductCode`, 
              `SampleType`, `SampleNumber`, `SampleComment`, `InstrumentName`, `InstrumentSerialNumber`,
              `Protein`, `Moisture`,`Ash`,`Wetgluten`, `Drygluten`, `SV`, `Infestation`, `HLWeight`, 
              `ForeignMatter`, `BadSmell`,`Dust`, `SeiveSize`,  `VANumber`)
                 VALUES ('$AnalysisTime','$ProductName','$ProductCode','$SampleType','$SampleNumber','$SampleComment',
                 '$InstrumentName','$InstrumentSerialNumber','$Protein','$Moisture','$Ash','$Wetgludten','$Drygluten',
                 '$SV','$Infestation','$HLWeight','$ForesightMatter','$bad_smell','$Dust','$seivesize','$VANumber')";
