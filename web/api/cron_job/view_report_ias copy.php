...
if (mysqli_num_rows($result) > 0) {
 //$fp = fopen('/home/nlsdp73/SAP/PP/ias/pp_sap_ias/' . $date . 'view_report_ias.csv', 'w');
    //$fname = '/home/nlsdp73/SAP/PP/ias/pp_sap_ias/' . $date . 'view_report_ias.csv'
    $fname = 'E:\\Foresight\\PurchasePro\\api\\cron_job\\Foresight\\' . $date . '_view_report_ias.csv';

    $fname = getcwd().'/CSV/' . $date . '_view_report_ias.csv';
    // echo $fname;
    $fp = fopen($fname, 'w');
    ...
    'QI_REFID',    'purchase_info_id',    'protein_quality',    'moisture_quality',    'ash_quality',    'wet_gluten_quality',    'dry_gluten_quality',    'sv_quality',    'hl_quality',    'kernel_quality',    'mm_quality',    'foreign_matter_quality',    'mudballs_quality',    'broken_wheat_quality',    'black_wheat_quality',    'soft_wheat_quality',    'shriveled_wheat_quality',    'immature_wheat_quality',    'insect_damage_wheat_quality',    'mixed_wheat_quality',    'kernel_bunt_quality',    'ofg_quality',    'infestation_quality',    'fungus_quality',    'rain_damage_quality',    'protein_type_quality',    'fn_quality',    'others_comment',    'overall_result',    'degrade',    'deduction_amount',    'wheat_variety',    'qc_work_doc',    'fungus_quality_noofbag',    'fungus_quality_quarantine',    'recommended_lot',    'rain_damage_quality_noofbag',    'rain_damage_quality_quarantine',    'red_grain_quality');
...
//exit();
       ...
       trim($row['QI_REFID']),    trim($row['purchase_info_id']),    trim($row['protein_quality']),    trim($row['moisture_quality']),    trim($row['ash_quality']),    trim($row['wet_gluten_quality']),    trim($row['dry_gluten_quality']),    trim($row['sv_quality']),    trim($row['hl_quality']),    trim($row['kernel_quality']),    trim($row['mm_quality']),    trim($row['foreign_matter_quality']),    trim($row['mudballs_quality']),    trim($row['broken_wheat_quality']),    trim($row['black_wheat_quality']),    trim($row['soft_wheat_quality']),    trim($row['shriveled_wheat_quality']),    trim($row['immature_wheat_quality']),    trim($row['insect_damage_wheat_quality']),    trim($row['mixed_wheat_quality']),    trim($row['kernel_bunt_quality']),    trim($row['ofg_quality']),    trim($row['infestation_quality']),    trim($row['fungus_quality']),    trim($row['rain_damage_quality']),    trim($row['protein_type_quality']),    trim($row['fn_quality']),    trim($row['others_comment']),    trim($row['overall_result']),    trim($row['degrade']),    trim($row['deduction_amount']),    trim($row['wheat_variety']),    trim($row['qc_work_doc']),    trim($row['fungus_quality_noofbag']),    trim($row['fungus_quality_quarantine']),    trim($row['recommended_lot']),    trim($row['rain_damage_quality_noofbag']),    trim($row['rain_damage_quality_quarantine']),    trim($row['red_grain_quality']));
       fputcsv($fp, $test, ",", " ");
