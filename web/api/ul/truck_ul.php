...
else if($entityULContent->formType === "LOT")
{
  echo getLotLocationDetails($connect,$entityULContent);
}
else if ($entityULContent->formType === "POS") {
  echo fetchlocation($connect, $entityULContent);
}else {
...
function getLotLocationDetails($connect, $record)
{
  //print_r($record);exit('aaa');
  $Qry ="SELECT PI_REFID,ZVA_NUMBER,WERKS,LGORT FROM `purchase_info` where ZVA_NUMBER='$record->RvNumbers'";

  $lotdet = getResultAsObjectArray($connect, $Qry);
 // $id=$lotdet[0]['PI_REFID'];
  $plantId=$lotdet[0]['WERKS'];
  //$storageLocation=$lotdet[0]['LGORT'];

  $Qry ="SELECT ID as plantID FROM `master_plant` where WERKS='$plantId'";
  //echo $Qry;
  $plant = getResultAsObjectArray($connect, $Qry);
  $plantID=$plant[0]['plantID'];


  //print_r($plant);exit('aaa');

  $Qry ="SELECT STORAGE_REFID FROM `master_storage` where LGORT='$$record->Location' and plantid='$plantID'";
  //echo $Qry;
  $plant = getResultAsObjectArray($connect, $Qry);
  $LocationID=$plant[0]['STORAGE_REFID'];

  $Qry ="SELECT lotno as value, lotno as label FROM `ngw_lot` where plantid='$plantID' OR locationid='$LocationID' and  RecStatus='1'";
  //echo $Qry;exit('test');
  $lotdet = getResultAsObjectArray($connect, $Qry);
  $poData['lotdet']=$lotdet;
 // print_r($record->Location);
  //print_r($poData);exit('aaa');

//print_r($poData);exit('aaaa');
...
function fetchPODetailsById($connect, $record)
{
  ...
  if(count($poData)>0){
    ...
    $WERKS = $poData[0]["FromWERKS"];

   $sql2 = "SELECT WB_QTY, WB_DT, ZSUPPLIER_INV_RATE, ZSUPPLIER_INV_NO, ZSUPPLIER_INV_DT, ZSUPPLIER_INV_QTY, INV_COPY, WB_COPY FROM supplier_vehical_info JOIN supplier_dispatch_info ON SUPPLIER_ID = SD_REFID WHERE (VEHICAL_NO = '".$vheNumber."' AND ZPO_NUMBER = '".$poNumber."' AND 	ZSUPPLIER_CODE = '".$supplierCode."' AND LINE_ITEM = '".$lineItem."' AND purchase_info_id = '".$record->id."') LIMIT 1";
    $formData = getResultAsObjectArray($connect, $sql2);
    //echo var_dump($formData);

    $sql10 = "
    SELECT svi.INV_COPY, svi.WB_COPY, svi.ZSUPPLIER_INV_RATE, svi.ZSUPPLIER_INV_NO, svi.ZSUPPLIER_INV_DT, svi.ZSUPPLIER_INV_QTY
    FROM supplier_vehical_info AS svi
    JOIN supplier_dispatch_info AS sdi
        ON svi.SUPPLIER_ID = sdi.SD_REFID
    WHERE sdi.SD_REFID = (
        SELECT MAX(SD_REFID)
        FROM supplier_dispatch_info
        WHERE ZPO_NUMBER = '".$poNumber."'
          AND ZSUPPLIER_CODE = '".$supplierCode."'
          AND ZPO_LINE_ITEM = '".$lineItem."'
    )
    LIMIT 1
    ";
    // print_r($sql10);exit();
    $formData10 = getResultAsObjectArray($connect, $sql10);

    $Qry5="SELECT Unloading_cost,Loading_cost,PURCHASE_ORG FROM `sap_to_pp` where EBELN='".$poNumber."' and EBELP='".$lineItem."'  and SUPPLIER_CODE='".$supplierCode."'  and WERKS='".$WERKS."'";
    
    $Unloading_cost = getResultAsObjectArray($connect, $Qry5);

    $poData[0]["Unloading_cost"] = $Unloading_cost[0]['Unloading_cost']; 
    $poData[0]["Loading_cost"] = $Unloading_cost[0]['Loading_cost'];
    $poData[0]["PURCHASE_ORG"] = $Unloading_cost[0]['PURCHASE_ORG'];

    if(count($formData)>0 || count($formData10)>0){
      $poData[0]["supplier_wb_qty"] = $formData[0]['WB_QTY'];
     // $poData[0]["invoice_rate"] = $formData[0]['ZSUPPLIER_INV_RATE'];
      $poData[0]["invoice_date"] = $formData[0]['ZSUPPLIER_INV_DT'] ? $formData[0]['ZSUPPLIER_INV_DT'] : $formData10[0]['ZSUPPLIER_INV_DT'];
      $poData[0]["invoice_no"] = $formData[0]['ZSUPPLIER_INV_NO'] ? $formData[0]['ZSUPPLIER_INV_NO'] : $formData10[0]['ZSUPPLIER_INV_NO'];
     // $poData[0]["invoice_qty"] = $formData[0]['ZSUPPLIER_INV_QTY'];
      $poData[0]["supplier_wb_dt"] = $formData[0]['ZSUPPLIER_INV_DT'];
      $poData[0]["INV_COPY"] = $formData[0]['INV_COPY'] ? $formData[0]['INV_COPY'] : $formData10[0]['INV_COPY'];
    ...
    // if($poData[0]["VEHICLE_TYPE"] == 'Rake'){
    $rake_detail ="SELECT a.receive_bag1,a.receive_bag2,a.receive_bag3,a.no_bags1,a.no_bags2,a.no_bags3,a.gunny_wt1,a.gunny_wt2,a.gunny_wt3,a.total_bags,a.total_gunny_wt,f.BAG_NAME as BAG_NAME,g.BAG_NAME
    as BAG_NAME2,h.BAG_NAME as BAG_NAME3,f.WEIGHT as bag_weight,g.WEIGHT
    as bag_weight2,h.WEIGHT as bag_weight3
     FROM `rake_loading` a
    ...
    where purchase_info_id='$record->id'";
    $rake_details = getResultAsObjectArray($connect, $rake_detail);

    $poData[0]['receive_bag1']=$rake_details[0]['receive_bag1'];
    $poData[0]['receive_bag2']=$rake_details[0]['receive_bag2'];
    $poData[0]['receive_bag3']=$rake_details[0]['receive_bag3'];
    $poData[0]['no_bags1']=$rake_details[0]['no_bags1'];
    $poData[0]['no_bags2']=$rake_details[0]['no_bags2'];
    $poData[0]['no_bags3']=$rake_details[0]['no_bags3'];
    $poData[0]['gunny_wt1']=$rake_details[0]['gunny_wt1'];
    $poData[0]['gunny_wt2']=$rake_details[0]['gunny_wt2'];
    $poData[0]['gunny_wt3']=$rake_details[0]['gunny_wt3'];
    $poData[0]['total_gunny_wt']=$rake_details[0]['total_gunny_wt'];
    $poData[0]['total_bags']=$rake_details[0]['total_bags'];
    $poData[0]['BAG_NAME']=$rake_details[0]['BAG_NAME'];
    $poData[0]['BAG_NAME2']=$rake_details[0]['BAG_NAME2'];
    $poData[0]['BAG_NAME3']=$rake_details[0]['BAG_NAME3'];
    $poData[0]['bag_weight']=$rake_details[0]['bag_weight'];
    $poData[0]['bag_weight2']=$rake_details[0]['bag_weight2'];
    $poData[0]['bag_weight3']=$rake_details[0]['bag_weight3'];
    }
...
function addUnloadingdetails($connect, $record)
{
  ...
  $gFields = ["bag_type","bag_type2","bag_type3", "no_bags","no_bags2","no_bags3", "gunny_wt", "unload_lot","invoice_date","invoice_no","invoice_qty","invoice_bag_count","invoice_rate","supp_inv_copy","supp_wb_copy","supplier_wb_dt","supplier_wb_qty","UnloadVendorName","UnloadVendorCharge","LoadingVendorID","LoadingCharge"];
...
function fetchlocation($connect, $record)
{
  // if(count($record)>0){
    //Receiving Plant Detail Fetch
    $toplantid = $record->toplantid;
    $tolocationid = $record->tolocationid;
    
    // print_r($toplantid);exit();
    // print_r($tolocationid);exit();
