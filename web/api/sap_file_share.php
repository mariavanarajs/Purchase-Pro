...
for ($i = 0; $i < $count_tmp_name_array; $i++) {
    ...
    if (!in_array(strtolower($extension), ['csv','pdf','jpeg','png','jpg'])) {
        ...
        // $path = str_replace("D:/Program Files/xampp/htdocs/", "http://localhost/", $filepath);
       ...
       // print_r($storepath);
        ...
        //$file_save_arr[$i]['updname_OLDSERVER']= str_replace("D:/Program Files/xampp/htdocs/", "http://localhost/", $storepath); //OLD SERVER //
        $file_save_arr[$i]['updname_OLDSERVER'] = str_replace('/var/www/purchasepro/sapfileshare', 'https://purchasepro.nagamills.com/sapfileshare',$storepath);        
        if($LocalTest==0){
            $file_save_arr[$i]['updname']= $storepath_NAS; //NAS SERVER PATH//Uncmt
        }else{
            //$file_save_arr[$i]['updname']= str_replace("D:/Program Files/xampp/htdocs/", "http://localhost/", $storepath); //OLD SERVER //Cmd
            $file_save_arr[$i]['updname'] = str_replace('/var/www/purchasepro/sapfileshare', 'https://purchasepro.nagamills.com/sapfileshare',$storepath); 
        }
        $file_save_arr[$i]['orgname']=$name_array[$i];
    }
}
