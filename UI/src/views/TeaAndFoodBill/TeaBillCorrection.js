...
import TeaBillList from './TeaBillList';
import { DatePicker } from '../forms/custom-datetime';
import moment from 'moment';

const TeaBillCorrection = () => {
    ...
    const getEmployeeDetails = (status) => {
        ...
        showLoader();
        apiGetMethod(apiBaseUrl + `FoodTeaTokenController/teaBillGet/${UserDetails.USERID > 1 ? UserDetails.GATE_ID : 0}/${status}/${fromDateMilliSecond}/${toDateMilliSecond}`)
            .then((response) => {
                const { data } = response;
    ...
    return (
        <>
            <Card>
            <CardHeader>Tea Bill Approval & Report</CardHeader>
                <hr></hr>
            <CardBody>
            ...
            <TeaBillList data={data} getEmployeeDetails={getEmployeeDetails} />
            </CardBody>
            </Card>
        </>
...
export default TeaBillCorrection