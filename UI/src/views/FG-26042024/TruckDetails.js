    ...
    const actionsCol = {
        ...
        cell: (row) => {
            ...
            return actionRendorer ? (
                ...
                {status == 1 && (row.SCREEN_TYPE == 'SDI' || row.SCREEN_TYPE == 'SDO')? 
                    <Button.Ripple
                    color="primary"
                    className='ml-1'
                    onClick={(e) => onUpdateStatusUnLoad(status, row.PI_REFID, row.INCO1, statusCode.UNLOAD)}
                    >
                    Gate In
                    </Button.Ripple>: null}
                {row.VEHICLE_STATUS == 1 && (row.SCREEN_TYPE == 'EVADP' || row.SCREEN_TYPE == 'SILOTOMILL') && status != 1 ? 
                    <Button.Ripple
                    color="primary"
                    className='ml-1'
                ...
                {status == 1 && (row.SCREEN_TYPE == 'IAS' || row.SCREEN_TYPE == 'SILOTOMILL')? 
                    <Button.Ripple
                    color="primary"
                    className='ml-1'
