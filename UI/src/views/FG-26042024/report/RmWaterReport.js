...
const RmWaterReport = ({ actionRendorer }) => {
    ...
    const getRmWaterReport = () => {
        ...
        showLoader();
        console.log(apiBaseUrl + `GatePro/Report/getRmWaterReport/${fromDateMilliSecond}/${toDateMilliSecond}/${userPlant}/${UserDetails.USERID}`)
        apiPostMethod(apiBaseUrl + `GatePro/Report/getRmWaterReport/${fromDateMilliSecond}/${toDateMilliSecond}/${userPlant}/${UserDetails.USERID}`)
            .then((response) => {
                const { data } = response;
