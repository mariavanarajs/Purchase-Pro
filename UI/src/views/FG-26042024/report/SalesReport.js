...
const SalesReport = () => {
    ...
    const getSalesReport = () => {
        ...
        let id = moduleTypeId != undefined ? moduleTypeId : 0
        let userPlant = form.values.masterPlantId != undefined ? form.values.masterPlantId.value : 0

        showLoader();
        console.log(apiBaseUrl + `GatePro/Report/getSalesReport/${fromDateMilliSecond}/${toDateMilliSecond}/${id}/${userPlant}/${UserDetails.USERID}`)
        apiPostMethod(apiBaseUrl + `GatePro/Report/getSalesReport/${fromDateMilliSecond}/${toDateMilliSecond}/${id}/${userPlant}/${UserDetails.USERID}`)
            .then((response) => {
                const { data } = response;
