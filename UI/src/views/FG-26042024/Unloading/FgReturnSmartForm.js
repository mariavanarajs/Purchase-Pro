        ...
        apiPostMethod(apiBaseUrl + `GatePro/Gate/getGateInInfo/0/0/0/${gateInOutInfoId}/${UserDetails.USERID}`)
            .then((response) => {
                ...
                if (data.success == true) {
                    ...
                    for (let i = 0; i < data.salesReturnInfo.length; i++) {
                        data.salesReturnInfo[i].sno = i + 1;
                    }   
                    setQuantityDetailsData(data.salesReturnInfo);
                }
            }).catch((error) => {
                                ...
                                <tbody>
                                    <tr>
                                        <td style={{ padding: "5px" }}><table style={{ width: "100%" }} border={0}>
                                            ...
                                            <tr style={trstyle}><td colSpan={6} style={subheadingstyle1}><strong>FG - SALES RETURN</strong></td></tr>
                                            <tr><td colSpan={6}><hr></hr></td></tr>

                                            <tr style={trstyle}>
                                                <td style={{ width: "20%" }}><strong>Vehicle Arrival No :</strong></td><td style={{ width: "15%" }}>{data.vaNumber}</td>
                                                <td style={{ width: "23%" }}><strong>Return Ref No :</strong></td><td style={{ width: "15%" }}>{data.returnRefNo}</td>
                                                <td><strong>Customer Name :</strong></td><td>{data.customerName}</td>
                                            </tr>

                                            <tr style={trstyle}>
                                                <td><strong>Vehicle No :</strong></td><td >{data.vehicleNo}</td>
                                                <td><strong>Driver Phone No. :</strong></td><td>{data.driverMobileNumber}</td>
                                                <td><strong>Receiving Plant :</strong></td><td>{data.plantName}</td>
                                            </tr>

                                            <tr style={trstyle}>
                                                <td><strong>Sales Invoice No :</strong></td><td >{data.salesInvoiceNo}</td>
                                                <td><strong>Sales Return Invoice No :</strong></td><td>{data.salesReturnInvoiceNo}</td>
                                            </tr>

                                            {quantityDetailsData.map(quantityDetails => (
                                                <tr style={trstyle}>
                                                    <td><strong>Material {quantityDetails.sno} :</strong></td><td>{quantityDetails.material}</td>
                                                    <td><strong>Return Qty {quantityDetails.sno} :</strong></td><td>{quantityDetails.returnQuantity}</td>
                                                </tr>
                                            ))}

                                            <tr style={trstyle}>
                                                <td><strong>Receiving GateIn Date & Time :</strong></td><td>{data.createdOn}</td>
                                                <td><strong>Receiving GateOut Date & Time :</strong></td><td>{data.gateOutDateStamp}</td>
                                            </tr>
                                        ...
                                        <tr>
                                            <td style={{ padding: "5px" }}>
                                                ...
                                                <table style={{ width: "100%" }} border={0}>
                                                    ...
                                                    <tr style={trstyle}>
                                                        <td style={{ width: "13%" }}><strong>Diff Weight :</strong></td><td style={{ width: "20%" }}>{Number(data.overAllDeliveryQuantity-data.netWeight).toFixed(2)}</td>
                                                    </tr>                                                    
                                                    <tr style={trstyle}>
...
export default FgReturnSmartForm