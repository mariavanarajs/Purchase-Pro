...
const RecieptInvoiceConfirmation = ({ actionRendorer }) => {
    ...
    const getLoadingData = () => {
        ...
        showLoader();
        apiPostMethod(apiBaseUrl + `MigoAutomationController/ReceiptDetailsGetByUsers/${3}/${UserDetails.USERID}`)
            .then((response) => {
                const { data } = response;
    ...
    return (
        <div>
                <Card >
                    <CardHeader><h5>Acknowledge Original Invoice - Accounts </h5></CardHeader>
                    <hr />
                    <CardBody>
            ...
            {show ? <RecieptEntryScreenApprovalDetails setShow={setShow} show={show} purchaseId={gateInOutInfoId} poNumbers={poNumbers} status={3}/> : null}
...
export default RecieptInvoiceConfirmation;
