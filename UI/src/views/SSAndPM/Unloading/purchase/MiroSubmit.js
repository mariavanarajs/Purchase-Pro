...
const MiroSubmit = ({ actionRendorer }) => {
    ...
    const getLoadingData = () => {
        ...
        showLoader();
        apiPostMethod(apiBaseUrl + `MigoAutomationController/getMiroDetailsList/${UserDetails.USERID}/${fromDateMilliSecond}/${toDateMilliSecond}/${2}/${UserDetails.plantids}`)
            .then((response) => {
                const { data } = response;
    ...
    return (
        <div>
                <Card >
                    <CardHeader><h5>MIRO ENTRY SUBMIT</h5></CardHeader>
                    <hr />
            ...
            {show ? <MiroSubmitDetails setShow={setShow} show={show} gateInOutInfoId={landingData[0].gateInOutInfoId} lines={lines} status={2} miroIds = {miroIds} getLoadingData = {getLoadingData} weighment = {landingData[0].firstWeight == null ? false : true}/> : null}
