...
const MiroSubmitApprovalSMG = ({ actionRendorer }) => {
    ...
    const getLoadingData = () => {
        ...
        showLoader();
        apiPostMethod(apiBaseUrl + `MigoAutomationController/getMiroApprovalDetailsList/${UserDetails.USERID}/${fromDateMilliSecond}/${toDateMilliSecond}/${6}/${UserDetails.plantids}`)
            .then((response) => {
                const { data } = response;
    ...
    return (
        <div>
                <Card >
                    <CardHeader><h5>INVOICE SUBMIT APPROVAL - 2</h5></CardHeader>
                    <hr />
...
export default MiroSubmitApprovalSMG;
