...
const RecieptInvoiceSubmit = ({ actionRendorer }) => {
    ...
    const getLoadingData = () => {
        ...
        showLoader();
        apiPostMethod(apiBaseUrl + `MigoAutomationController/ReceiptDetailsGetByUsers/${2}/${UserDetails.USERID}/${fromDateMilliSecond}/${toDateMilliSecond}`)
            .then((response) => {
                const { data } = response;
    ...
    const getModuleType = () => {
        apiGetMethod(apiBaseUrl + `GatePro/Master/getModuleType/0/${UserDetails.USERID}`)
            .then((response) => {
                const { data } = response;
                if (data.success == true) {
                    setModuleTypeData(data.results.filter((item) => item.moduleTypeId == 2 || item.moduleTypeId == 12 || item.moduleTypeId == 15 || item.moduleTypeId == 21 || item.moduleTypeId == 25 || item.moduleTypeId == 33 || item.moduleTypeId == 34))
                }
                else if (data.success == false) {
                    errorToast(data.message)
                }
    ...
    return (
        <div>
                <Card >
                    <CardHeader><h5>Submit Original Invoice - Stores to Accounts</h5></CardHeader>
                    <hr />
                    <CardBody>
                        <Row>
                        <Col md="3" sm="3">
            ...
            {show ? <RecieptEntryScreenApprovalDetails setShow={setShow} show={show} purchaseId={gateInOutInfoId} poNumbers={poNumbers} status={2}/> : null}
...
export default RecieptInvoiceSubmit;
