...
const RecieptEntryScreenMGApproval = ({ actionRendorer }) => {
    ...
    const getLoadingData = () => {
        ...
        showLoader();
        apiPostMethod(apiBaseUrl + `MigoAutomationController/ReceiptDetailsGetByUsers/${1}/${UserDetails.USERID}`)
            .then((response) => {
                const { data } = response;
    ...
    return (
        <div>
                ...
                <Card >
                    <CardHeader><h5>MIGO - Manager Approval</h5></CardHeader>
                    <hr />
                    <CardBody>
            ...
            {show ? <RecieptEntryScreenApprovalDetails setShow={setShow} show={show} purchaseId={gateInOutInfoId} poNumbers={poNumbers} status={1}/> : null}
...
export default RecieptEntryScreenMGApproval;
