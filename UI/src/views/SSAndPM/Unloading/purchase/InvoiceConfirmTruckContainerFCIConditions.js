...
const InvoiceConfirmTruckContainerFCIConditions = ({ }) => {
    ...
    const amountDebugCounterRef = useRef(0);
    ...
    const getGateInInfo = () => {  
        ...
        // } 
        showLoader();
        apiPostMethod(apiBaseUrl + `MigoAutomationController/getMiroDetailsByIdConditionsWheat`, requestData)  // Send as body, not in URL
            .then((response) => {
                const { data } = response;
                // response success flag
                if (data.success === true) {
                    // snapshot baseline totals (amount + totalTax) into each row as _baselineTotal
                    const mapped = (data.results || []).map((r, idx) => {
                        const baseline = Number(r.amount || 0) + Number(r.totalTax || 0);
                        // store in a ref map keyed by id or index for quick lookup
                        const key = r.id != null ? r.id : idx;
                        baselineTotalsRef.current[key] = baseline;
                        return { ...r, _baselineTotal: baseline };
                    });
                    setPoData(mapped);
    ...
    // attached files info logged in UI when needed

       if (keys.length == 0 && filteredPoData[0]?.invoiceCopy) {
    ...
    const AddDatasPO = () => {
        ...
        const filteredPoData = poData.filter(item => item.selected === true);
            // filteredPoData available for processing
            if (!filteredPoData.length) {
                errorToast('Please select at least one MIRO line');
                return;
            }
            ...
            // condition and invoice sets processed
    ...
    const update = (shipmentCopys,ExtraAttachments) => {
        ...
        let postData = {
            ...
            shipmentCopy:shipmentCopys || filteredPoData[0]?.invoiceCopy,
        ...
        confirmDialog({
            ...
            description: `Selected Rows: ${selectedRowCount} | Overall GRN Qty: ${totalQty} | Total Amount: ${postData.totalAmount}`
        }).then((res) => {
            if (res) {
    ...
    // selectedOptions processed
      ...
      const GetPODetails = () => {
        ...
        apiPostMethod(apiBaseUrl + `MigoAutomationController/getPoNumbersWheat/${UserDetails.USERID}/${fromDateMilliSecond}/${toDateMilliSecond}/${UserDetails.plantids.length > 0 ? UserDetails.plantids : 0}/${0}`)
          .then((response) => {
    ...
    // Validation tolerance used by amount change handler
    ...
    const handleAmountChange = (index, value) => {
        ...
        // prefer stored baseline on the row, else fallback to ref map or compute
        const baselineFromRow = row._baselineTotal !== undefined ? Number(row._baselineTotal) : null;
        const rowKey = row.id != null ? row.id : index;
        const baselineFromRef = baselineTotalsRef.current[rowKey];
        const baselineTotal = baselineFromRow !== null ? baselineFromRow : (baselineFromRef !== undefined ? baselineFromRef : (Number(row.amount) || 0) + Number(row.totalTax || 0));

        const lowerTotal = baselineTotal * (1 - AMOUNT_TOLERANCE);
        const upperTotal = baselineTotal * (1 + AMOUNT_TOLERANCE);

        const minAllowedAmount = Math.max(0, lowerTotal - totalTax);
        const maxAllowedAmount = upperTotal - totalTax;

        
        if (num > maxAllowedAmount) {
            errorToast(`Amount cannot exceed ${maxAllowedAmount.toFixed(2)} (10% limit)`);
            updateValue(index, 'amount', maxAllowedAmount.toFixed(2));
            return;
        }

        // valid — update state keeping user's raw input (or formatted)
        updateValue(index, 'amount', sanitized);
    };

    // debug limiter to avoid excessive logs in large tables
    // (no longer used)
    
    const exportToExcel = () => {
    // poData logged when needed
...
export default InvoiceConfirmTruckContainerFCIConditions;
