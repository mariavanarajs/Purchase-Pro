...
const TrailMaterialDetails = () => {
    ...
    const getLoadingAndUnloadingInfo = () => {
        showLoader();
        console.log(apiBaseUrl + `GatePro/Gate/getGateInInfo/0/0/0/${gateId}/${UserDetails.USERID}`);
        apiPostMethod(apiBaseUrl + `GatePro/Gate/getGateInInfo/0/0/0/${gateId}/${UserDetails.USERID}`)
            .then((response) => {
                const { data } = response;
                if (data.success == true) {
                    setData(data.results[0]);
                    let loadingData = data.results[0]
                    getPurchaseOrder(loadingData.loadingUnloadingInfoId)
                    form.setValues({
                        invoiceNo: loadingData.invoiceNo,
                        vehicleNo: loadingData.vehicleNo,
                        masterPlantId: loadingData.masterPlantId,
                        driverMobileNumber: loadingData.driverMobileNumber,
    ...
    const checkInvoiceNo = () => {
        ...
        if ((invoiceNo == "") && (data.moduleTypeId == 11 || data.moduleTypeId == 12 || data.moduleTypeId == 13 || data.moduleTypeId == 15 || data.moduleTypeId == 21 || data.moduleTypeId == 33 || data.moduleTypeId == 34 || data.moduleTypeId == 16)) {
            confirmDialog({
            ...
            const checkPoNo = poData.filter((poData) => poData.invoiceNo == invoiceNo);
            if (checkPoNo != '') {
                confirmDialog({
    ...
    const updatePurchaseOrderDetails = () => {
        ...
        showLoader();
        console.log(apiBaseUrl + "GatePro/Master/updateTrailPurchaseOrderDetails", FrmData)
        apiPostMethod(apiBaseUrl + "GatePro/Master/updateTrailPurchaseOrderDetails", FrmData)
            .then((response) => {
                const { data } = response;
                if (data.success == true) {
    ...
    const deletePurchaseOrderDetails = (purchaseOrderId) => {
        setPoNo('PO Number')
        const postdata = {
    ...
    return (
        <form >
            <Fragment>
                <Card>
                    <CardHeader><h5>Trail Material Info</h5><RefreshBlock1 /></CardHeader>
                    <hr />
                    <CardBody>
                        <Row>
                            {data?.moduleType ?
                            ...
                            {data?.vaNumber	 ?
                                <Col md="4" sm="4">
                                    <CustomTextInput label={"VA Number"} form={form} id="vaNumber" type="text" value={data?.vaNumber} maxLength={10} disabled />
                                </Col> : null
                            }
                            {data?.vehicleNo	 ?
                                <Col md="4" sm="4">
                                    <CustomTextInput label={"Truck No"} form={form} id="vehicleNo" type="text" value={form.values.vehicleNo} maxLength={10} disabled />
                                </Col> : null
                            }
                            {data?.driverMobileNumber	 ?
                                <Col md="4" sm="4">
                                    <CustomTextInput label={"Driver Mobile No"} form={form} id="driverMobileNumber" type="text" value={form.values.driverMobileNumber} maxLength={10} disabled/>
                                </Col> : null
                            }
                            ...
                            {data?.plantName ?
                                ...
                                {data?.firstWeight ?
                                <Col md="4" sm="4">
                                    <CustomTextInput label={"First Weight"} form={form} id="truckNo" type="text" value={data?.firstWeight} disabled />
                                </Col> : null
                                }
                                {data?.secondWeight?
                                    <Col md="4" sm="4">
                                        <CustomTextInput label={"Second Weight"} form={form} id="vehicleNo" type="text" value={data?.secondWeight} maxLength={10} disabled />
                                    </Col> : null
                                }
                                {data?.netWeight?
                                    <Col md="4" sm="4">
                                        <CustomTextInput label={"Net Weight"} form={form} id="driverMobileNumber" type="text" value={data?.netWeight} maxLength={10} disabled/>
                                    </Col> : null
                                }
                                {data?.moduleTypeId == 41 ? <>
                            ...
                            {poDetails != '' ? <>
                                {(showPoDetails && poDetails != '') && (data.moduleTypeId == 41) ?
                                    <Col md="4" sm="4">
                                    ...
                                    <table className="table table-bordered">
                                        <thead >
                                            <tr>
                                                ...
                                                {invoiceNo ? <th className="bg-primary text-white">Invoice No /<br /> Delivery Challan</th> : null}
                                        ...
                                        {poData.map((poDetailsData) => (
                                            <tbody key={poDetailsData.poNumber}>
                                                <tr>
                                                    <td>{poDetailsData?.poNumber}</td>
                                                    {invoiceNo ? <td>{poDetailsData?.invoiceNo}</td> : null}
                                                    ...
                                                    {poDetailsData?.purchaseOrderId == undefined ? <td className="text-center"><X size={16} onClick={() => handleremove(poDetailsData?.poNumber)} /></td> : <td className="text-center text-danger"><Trash size={16} onClick={() => deletePurchaseOrderDetails(poDetailsData?.purchaseOrderId)} hidden = {poDetailsData?.migoNumber != null}/></td>}
                                                </tr>
                                            </tbody>
                                        ))}
                                    </table>
                        ...
                        <Row>
                            ...
                            <Col sm="4" md="4">
                                <FormGroup className="d-flex justify-content-end mb-0 mt-2">
                                    <Button.Ripple color="primary" type="button" onClick={() => updatePurchaseOrderDetails()}>
...
export default TrailMaterialDetails;
