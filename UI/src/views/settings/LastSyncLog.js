...
import LastSyncList from "../List/LastSyncList";
import { LastSyncUrl } from "../../urlConstants";
const LastSyncLogForm = ({ form,onSubmit }) => {
  ...
  return (
    <Fragment>
      <Row>
        ...
        <Col md="4" sm="12">
          <CustomDropdownInput url={`${apiBaseUrl}marketdata/master/getFromLocation`} label={"From Location"} form={form} id="FromLocation" />
        </Col>
        <Col md="4" sm="12">
        <CustomDropdownInput url={`${apiBaseUrl}marketdata/master/getToLocation`} label={"To Location"} form={form} id="ToLocation" />
        </Col>
      </Row>
      <Row>
      <Col md="4" sm="12">
        <CustomDropdownInput url={`${apiBaseUrl}marketdata/master/getModeOfTransport`} label={"Mode of Transport"} form={form} id="ModeofTransport" />
        </Col>
        <Col md="4" sm="12">
          <CustomTextInput label={"Ead"} form={form} id="Ead" type="text"  />
        </Col>
      </Row>
      <Row>
      <Col md="2" sm="12">
 ...
 // const today = moment().format(dateFormat);
  ...
  const form = useFormik({
    ...
    validationSchema: Yup.object().shape({
      ...
      From_Location: validation.required({  message:"From Location should not be empty",isObject: false }),
 ...
 // alert("1");
   ...
   /* if (poData.LINE_ITEM === formData.LINE_ITEM.value) {
    ...
    const FrmData = {
      ...
      date:formData.date,
      From_Location:formData.FromLocation.label,
      To_Location:formData.ToLocation.label,
      Mode_Of_Transport:formData.ModeofTransport.label,
      EAD:formData.Ead,
    };
    const postdata = {
      id:formData.EadId,
  ...
  return  <Button.Ripple color="primary" onClick={(e) => window.open(row.url)} >
  {"Action"}
</Button.Ripple>
  ...
  return (
    <Fragment>
      {false ? <CardComponent header="Ead">
        <LastSyncLogForm form={form}  onSubmit={onSubmit} />
      </CardComponent>: ""}
      <LastSyncList
