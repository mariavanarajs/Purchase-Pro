...
let IasSendingReceiving = () => {
          ...
          if (arrData.data.success && disData.data.success) {
            ...
            if (disData.data.results) {
              ...
              newVal = {
                ...
                pickSlipNo: pickOp,
                sendingWbType: getSelectedWbOption(wbType),
              ...
              if(rest.OwnWBID && rest.OwnWBID>0){
                newVal = {
                  ...newVal,
                  wbType: getSelectedWbOption("OWN BY"),
                }
                setTicketdata([{ options: rest.TicketDetails }]);
              }
            }
  ...
  const onReceiverUpdateGateOut = () => {
    ...
    if (postData.nagaOutsideWBCopy) {
      uploadFile(emptyArrivalId, postData.nagaOutsideWBCopy, (file) => {
        if (file) {
          postData.nagaOutsideWBCopy = file;
          addOrUpdateGateOut(postData, onSuccess);
        } else {
          hideLoader();
        }
      });
      ...
      uploadFile(emptyArrivalId, postData.WBCopy, (file) => {
        if (file) {
  ...
  const onGateOutDetails = () => {
    ...
    uploadIfAnyFileExist(emptyArrivalId, values, filesToUpload, (fileData) => {
      if (!fileData) {
        hideLoader();
      } else {
        if (dispatchId) {
          ...
          <Row>
            ...
            {(dispatchDetails || !isTruck) && (
              <>
                ...
                <Col md="4" sm="12">
                      <CustomTextInput label={"Own WB - Weight"} form={form} id="WBCalcWeight" isNumberOnly />
                      <CustomTextInput form={form} id="WBBufferPercent" isNumberOnly  style={{display:  'none' }} />
                </Col>
                <SendingWeightDetails form={form} />
                {isReceivingGateOut && (
                  <>
                    <HrLine header={"Receiving WB Details"} />
                    <WbTypeSelection form={form} />
            ...
            <Col sm="12" className="mt-2">
              <FormGroup className="d-flex mb-0 justify-content-end">
                <div className="mr-1">
                  ...
                  <Button.Ripple color="primary" type="button" id="RejectButton" onClick={(e) => onRejectGateOut()}>
                     Reject
                    </Button.Ripple>
                    &nbsp; &nbsp;
