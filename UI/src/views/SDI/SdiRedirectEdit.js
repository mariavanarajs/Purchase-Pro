...
const SdiRedirectEdit = () => {
  ...
  const onFetchSDIdetailsById = () => {
    ...
    showLoader();
    apiPostMethod(apiBaseUrl + "sdi/getsdiDetailsById", fdata)
      .then((response) => {
        const { data } = response;
        if (data.success) {
  ...
  const onSubmitSD = () => {
    ...
    showLoader();
    apiPostMethod(apiBaseUrl + "sdi/updatesdiPOLine", postdata)
      .then((response) => {
        const { data } = response;
        if (data.success) {
          resetForm();
  ...
  const renderSupplierInvoiceDetails = (info) => {
    return (
      <>
        {getTextElement("Invoice Rate (In Kgs)", info.ZSUPPLIER_INV_RATE, "3")}
        {getTextElement("Invoice No", info.ZSUPPLIER_INV_NO, "2")}
        {getTextElement("Invoice Date", info.ZSUPPLIER_INV_DT, "2")}
        {getTextElement("Invoice Qty (In Kgs)", info.ZSUPPLIER_INV_QTY, "3")}
      </>
    );
  };
              ...
              <div key={vehicleId + "_" + i}>
                <Row className="col-12 p-0 m-0">
                  ...
                  {vehicleId === TruckMdl || vehicleId === ContrMdl ? (
                    <>
                      {getTextElement("Supplier WB qty (In Kgs)", item.WB_QTY, "3")}
                  ...
                  {renderSupplierInvoiceDetails(item)}
                  <Col md="2" sm="12">
                    <FormGroup>
        ...
        <CardBody>
          <div>
            <Row>
              ...
              <Col md="4" sm="12">
                <CustomDropdownInput options={poLineOptions} label={"PO Line Item"} form={form} id="LINE_ITEM" />
...
export default SdiRedirectEdit;
