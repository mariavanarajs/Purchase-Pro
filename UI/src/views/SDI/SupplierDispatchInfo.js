...
const SupplierDispatchInfo = () => {
  ...
  const onSubmitSD=()=>{
    ...
    if((formData.FNR_NO == '' || formData.FNR_NO == undefined || !/^[0-9]{11,12}$/.test(formData.FNR_NO)) && (poData.VEHICLETYPE == 'Rake' || poData.VEHICLETYPE == 'RAKE')){
      errorToast('Please Enter Correct FNR NO...')
      return false
    }
  ...
  const onSubmitSD_SAVE = () => {
   ...
   // return false;
    ...
    postdata.append("IDNLF", poData.IDNLF);
    postdata.append("INCO1", poData.INCO1);
    // postdata.append("FNR_NO", formData.FNR_NO);
    postdata.append("LINER_NAME", selectedLiner ? selectedLiner.label : "");
    if (allKeys && allKeys.length) {
      allKeys.forEach((key) => {
        postdata.append([key], formData[key]);
        ...
        <CardBody>
          <div>
            <Row>
              ...
              {(VEHICLETYPE == "RAKE" || VEHICLETYPE == "Rake" || VEHICLETYPE === "rake")&&
              <Col md="4" sm="12">
                <FormGroup>
                  <Label>FNR NO</Label>
                  <Input
                    ...
                    id={"FNR_NO"} 
                    value={FNR_NO}
                    onChange={(e) => onTextChange(e, "FNR_NO")}
                    />
                </FormGroup>
               </Col>
              }
