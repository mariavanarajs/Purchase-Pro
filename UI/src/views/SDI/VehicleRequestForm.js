...
const VehicleRequestForm = () => {
    ...
    const getGateInInfo = () => {
        console.log(apiBaseUrl + `GatePro/Master/SupplierVehicleInfoById/${ID}`);
        apiPostMethod(apiBaseUrl + `GatePro/Master/SupplierVehicleInfoById/${ID}`)
            .then((response) => {
                const { data } = response;
                                ...
                                <tbody>
                                    <tr>
                                        <td style={{ padding: "5px" }}><table style={{ width: "100%" }} border={0}>
                                            ...
                                            <tr style={trstyle}><td colSpan={6} style={subheadingstyle1}><strong>{'Vehicle Request (' + 'Purchase' + ')'}</strong></td></tr>
                                            <tr><td colSpan={6}><hr></hr></td></tr>

                                            <tr style={trstyle}>
                                                <td><strong>Movement Type :</strong></td><td>{'Unloading'}</td>
                                                <td style={{ width: "20%" }}><strong>Sub Movement Type :</strong></td><td style={{ width: "15%" }}>{data.VEHICLE_TYPE ? data.VEHICLE_TYPE : 'Vehicle'}</td>
                                                <td style={{ width: "22%" }}><strong>Vehicle No :</strong></td><td >{data.VEHICAL_NO}</td>
                                            </tr>
                                            <tr style={trstyle}>
                                                <td><strong>Loading Point :</strong></td><td>{data.ZSUPPLIER_LOAD_POINT}</td>
                                                <td><strong>Plant :</strong></td><td>{data.WERKS}</td>
                                                <td><strong>ZSUPPLIER INV NO :</strong></td><td>{data.ZSUPPLIER_INV_NO}</td>
                                            </tr>
                                            <tr style={trstyle}>
                                                <td><strong>PO Number :</strong></td><td>{data.ZPO_NUMBER}</td>
                                                <td><strong>Supplier Name :</strong></td><td>{data.ZSUPPLIER_NAME}</td>
                                                <td><strong>Vehicle Request Date :</strong></td><td>{data.DateAdded}</td>
                                            </tr>
                                        </table>
                                        </td>
                                    </tr>
