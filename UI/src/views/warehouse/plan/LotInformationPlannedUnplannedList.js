...
export const LotInformationPlannedUnplannedLists = ({ title, url, actionRendorer, postData, screenType, ...rest }) => {
  ...
  const handleShowModal =(WarehouseId)=>{
    ...
    const ReceivingBins = {
      ...
      cell: (row) => {
        return  (
            <CustomInput type="text"  id="ReceivingBin"  disabled/>
        );
      },
    }; 
...
export const LotInformationPlannedUnplannedList  = ({ form }) => {
  const history = useHistory();
