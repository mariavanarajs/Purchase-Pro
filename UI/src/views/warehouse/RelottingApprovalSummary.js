...
const RelottingEntrySummaryForm = ({ form,onSubmit }) => {
	 ...
	 const taColumns = [
    {
	  ...
	  const actionsCol = {
		...
		cell: (row) => {
		  return  (
			<Button.Ripple color="primary"  onClick={() => ViewRelot(row.RelotId,row.RelotStatus)} >
			  {"Approve"}
			</Button.Ripple>
		  );
		},
	  };
	  const ViewRelot = (RelotId,RelotStatus) => {
     
        history.push("/warehouse/RelottingApprovalScreen:" + RelotId);
  ...
  const RefreshPage = () => {
    history.push(`/warehouse/masters/RelottingApprovalSummary`);
  };
  ...
  return (
    <Fragment>
       <Card>
        <CardHeader>
          <CardTitle>Relotting - Approval Screen 			 </CardTitle>
        </CardHeader>
        <CardBody>
          <TableComponent columns={columns} url={RelottingUrl} formType="getRelottingforApproval" />
        </CardBody>
      </Card>
...
const RelottingApprovalSummary = () => {
      ...
      .then((response) => {
        ...
        if(UsrId==-5){
          ...
          if(RespId && RespId>=1)
          {
            ...
            {
              history.push("/warehouse/masters/RelottingApprovalSummary:0");
            }
            else
            {
              history.push("/Warehouse/Masters/RelottingApprovalSummary");
  ...
  export default RelottingApprovalSummary;
