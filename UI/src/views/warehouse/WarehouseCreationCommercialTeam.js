...
import { apiBaseUrl, vaUrl, uploadUrl } from '../../urlConstants'
...
const WarehouseCreationCommercialTeam = ({form,onSubmit, setTitleName,isViewOnly}) => {
  const [attachedFiles, setAttachment] = useState({ naga_pwh_insurance_attachment: {}, contract_agreement_attachment: {}});
  if(!isViewOnly)
  {
    isViewOnly=false;
  }
     ...
     .then((response) => {
       ...
       if (data.success) {
        form.setValues({
          ...
          naga_pwh_insurance_attachment:(isViewOnly==true ? data.results[0].naga_pwh_insurance_attachment:""),
          ...
          contract_agreement_attachment:(isViewOnly==true ? data.results[0].contract_agreement_attachment:""),
          GST_TYPE:data.results[0].gst_type,
          effective_from:data.results[0].effectiveFrmDt,
          effective_to:data.results[0].effectiveToDt
         });
         sumitdisabled=false;
        //  Dijo 01 renamed Col 67 (bankname to name_of_bank) & (companyName to company_name)
        //  form.setFieldValue("name_of_bank",{label:data.results[0].name_of_bank,value:data.results[0].name_of_bank})
        //  form.setFieldValue("company_name",{label:data.results[0].company_name,value:data.results[0].company_name})
        form.setFieldValue("name_of_bank",{label:data.results[0].name_of_bank})
        form.setFieldValue("company_name",{label:data.results[0].companyName})
    ...
    const LotView = ()=>{
      ...
      history.push(`/warehouse/viewwcllotmgrapprove:`+refid);
    }
...
const SaveCommTeamEntry = () => { 
  ...
  const FrmData = { 
    ...
    // Dijo 02 changed on Col 39 .value to .label
    company_name:formData.company_name.label,
    name_of_bank:formData.name_of_bank.label,
    // End
  ...
  apiPostMethod(apiBaseUrl + "warehouse/master/SaveWarehouseUpdate", postdata)
    .then((response) => {
      ...
      if(RespId && RespId>=1)
      {
        ...
        history.push("/warehouse/wclcomm");
      }
      else
      {
        if(data.ErrorMsg)
    ...
    return (
         <Fragment>
             <Row>
              ...
              <Row>
                 ...
                 disabled={isViewOnly && ((isViewOnly==true && "true")|| (isViewOnly==false && "false")) } 
                 /> 
                 ...
                 <Select
                    ...
                    <CustomDropdownInput  url={`${apiBaseUrl}warehouse/master/getbankDetails`} form={form} label={"Name Of The Bank"} id="name_of_bank"
                    ...
                    <Col md="3" sm="12">
                    <CustomTextInput label={"NAGA/PWH Insurance No"} form={form} id="naga_pwh_insurance_no" type="text"  
                    disabled={isViewOnly && ((isViewOnly==true && "true")|| (isViewOnly==false && "false")) }
                    /> 
                    ...
                    <Uploader 
                    title={"Attachment"} 
                    label={"NAGA/PWH Insurance"}
                    isReadOnly={isViewOnly}
                    form={form} 
                    id={"naga_pwh_insurance_attachment"} 
                    setAttachment={handleFileChange}
                    selectedFileName={form.values.naga_pwh_insurance_attachment} />
                    ...
                    {isViewOnly==true && <Uploader isReadOnly={true} label={"NAGA/PWH Insurance"} selectedFileName={form.values.naga_pwh_insurance_attachment} />}
                    </Col>  
             ...
             <Row>
              ...
              <Row>
                  <Col md="3" sm="12">
                  <CustomTextInput label={"Insurance Start Date"} form={form} id="insurance_start_date" type={isViewOnly==false?"Date":"text"}  
                  disabled={isViewOnly && ((isViewOnly==true && "true")|| (isViewOnly==false && "false")) }
                   /> 
                  <span id="insurance_start_date_Error" style={{color: "red"}} ></span>
                  </Col> 
                  <Col md="3" sm="12">
                  <CustomTextInput label={"Insurance End Date"} form={form} id="insurance_end_date" type={isViewOnly==false?"Date":"text"}  
                  disabled={isViewOnly && ((isViewOnly==true && "true")|| (isViewOnly==false && "false")) }
                   />
              ...
              <Col md="3" sm="12">
                    ...
                    isReadOnly={isViewOnly}
                    form={form} 
                    id={"contract_agreement_attachment"} 
                    setAttachment={handleFileChange}
                    selectedFileName={form.values.contract_agreement_attachment} />

                    {/*<Button.Ripple color="primary"  type="Button" >Capture</Button.Ripple>*/}
                    <span id="contract_agreement_attachment_Error" style={{color: "red"}} ></span>
                    </>}

                    {isViewOnly==true && <Uploader isReadOnly={true} label={"Contract Agreement"} selectedFileName={form.values.contract_agreement_attachment} />}
                    </Col>  
            ...
            <Row> 
                ...
                <Select
                    id={"GST_TYPE"}
                onChange={"gst_type"}/>           */}
                <CustomTextInput label={"GST Type"} form={form} id="GST_TYPE" type="text" 
                disabled={isViewOnly && ((isViewOnly==true && "true")|| (isViewOnly==false && "false")) }
                 /> 
                 {/* <span id="gst_type_Error" style={{color: "red"}} ></span> */}
                </Col> 
                <Col md="3" sm="12">
                    <CustomTextInput label={"Effective Form"} form={form} id="effective_from" type={isViewOnly==false?"Date":"text"} 
                    disabled={isViewOnly && ((isViewOnly==true && "true")|| (isViewOnly==false && "false")) }
                     />
                    <span id="effective_from_Error" style={{color: "red"}} ></span>
                    </Col>  
                    <Col md="3" sm="12">
                    <CustomTextInput label={"Effective To"} form={form} id="effective_to" type={isViewOnly==false?"Date":"text"} 
                    disabled={isViewOnly && ((isViewOnly==true && "true")|| (isViewOnly==false && "false")) }
                    /> 
                    <span id="effective_to_Error" style={{color: "red"}} ></span>
                    </Col> 
                    {console.log("isViewOnly Value : "+ JSON.stringify(isViewOnly))}    
                    {isViewOnly==false && <Col md="3" sm="12">
                    <Button.Ripple color="primary"  type="Button" isDisabled={sumitdisabled} className='mt-2' onClick={(e) => SaveCommTeamEntry(e)}>Submit</Button.Ripple>
                    </Col>}
            </Row>
         ...
         <Button.Ripple color="primary"  type="Button"> View Entry</Button.Ripple>
          ...
          <Button.Ripple color="primary"  type="Button" 
           onClick={() => LotView()}
      > View Warehouse Lot</Button.Ripple>
          </div>
         </div>
...
const WarehouseCreationCommercialTeamData = ({isViewOnly}) => { 
    ...
    const form = useFormik({
      ...
      //  naga_pwh_insurance_no: validation.required({ message:"Naga Pwh Insurance No should not be empty", isObject: true }),
        ...
        companyName: validation.required({ message:"Company Name Date should not be empty", isObject: false }),
