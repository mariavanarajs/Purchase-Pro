...
const TargetVsActual = ({form,onSubmit}) => { 
    ...
    */
    const onFetchStockentryById = (e) => {
      let SrchVal=e ? e.target.value : "";
      let fdata = {
        Screen:"TARGETVSACTAL",
        Search:SrchVal,
        FromDate:form.values.FromDate?form.values.FromDate:"",
        ToDate:form.values.ToDate?form.values.ToDate:"",
        warehouseId:form.values.warehouseid ?form.values.warehouseid.value :"",
        PlantId:form.values.locationid?form.values.locationid.value : "",
        WheatVarietyId:form.values.wheatvarietyid ? form.values.wheatvarietyid.value: "",
        StorageLocationId:form.values.storagelocationid ? form.values.storagelocationid.value: ""
      };
    showLoader();
    //console.log("Request Url :: "+apiBaseUrl + "Master/getstockEntrydatabyid", fdata);
     apiPostMethod(apiBaseUrl + "warehouse/reports/wmTargetVsActual", fdata)
     .then((response) => {
       const { data } = response;
    ...
    const ViewRndLot = (sub_lot_id) => {
  
      history.push("/warehouse/TargetVsActual:"+sub_lot_id);
  }
...
const fileUploadAction = (id) => {
 ...
 const FillWheatVarityList = (StorageLocationId) => {
  let fdata = { WareHouseId:form.values.warehouseid.value,
    PlantId:form.values.locationid.value,
    StorageLocationId, screenType: "RND" };
  apiPostMethod(apiBaseUrl+'warehouse/master/getSublotWHeatvarietyList', fdata)
  .then((response) => {
  const { data } = response;
  if (data.success) {
...
// console.log("Request Url :: "+apiBaseUrl + "warehouse/getbagcuttingEntrydatabyid", fdata);
 apiPostMethod(apiBaseUrl + "warehouse/RndlotConversion/getLotConvertionDetails", fdata)
 .then((response) => {
   const { data } = response;
   console.log("Response Data :: "+JSON.stringify(response));
   if (data.success) {
...
<Col md="3" sm="12">
          ...
          <Col md="3" sm="12"> 
             ...
             <div className='d-flex'>
              <div className='p-2'>
              <Button.Ripple onClick={onFetchStockentryById}  color="primary"  type="Button"  >
             Show
             </Button.Ripple>
             </div> 
             <div className='p-2'>
            <Button.Ripple color="primary" style={{width:"110px"}} type="Button" onClick={ExcelExport} >Excel Export</Button.Ripple>  
             </div> 
             </div> 
        ...
        <Row> 
            ...
            <CustomTextInput label={"Search"}
              onChange={(e) => onFetchStockentryById(e)}
            form={form} id="Search" type="text"  />
            </div>
            </Col> 
        ...
        <Row> 
         <Col md='12' sm="12" style={{width:"120%",overflowX:"scroll",fontSize:"12px"}}> 
        {/*<div style={{width:"120%",overflowX:"scroll",fontSize:"12px"}}>*/}
        <table responsive id="TableID" className='table-sm'> 
            <thead className='bg-primary text-white' style={{height:"50px",textAlign:"center",fontSize:"12px"}} > 

                <tr> 
                    <th style={{backgroundColor:"#7367f0",minWidth:"200px",fontWeight:"500"}} >Warehouse Code</th> 
                    
                    <th style={{backgroundColor:"#7367f0",minWidth:"200px",fontWeight:"500"}} >Warehouse Name</th> 
                    <th style={{backgroundColor:"#7367f0",minWidth:"200px",fontWeight:"500"}} >Plant</th> 
                    <th style={{backgroundColor:"#7367f0",minWidth:"200px",fontWeight:"500"}} >Storage Location </th> 
                   ...
                   <th style={{backgroundColor:"#7367f0",minWidth:"200px",fontWeight:"500"}}>Target Qty</th>
                   <th style={{backgroundColor:"#7367f0",minWidth:"200px",fontWeight:"500"}}>Actual Qty</th>
                   <th style={{backgroundColor:"#7367f0",minWidth:"200px",fontWeight:"500"}}>% Achieved </th>
                   <th style={{backgroundColor:"#7367f0"}}>Balance </th>
                    {form.values.Dates && form.values.Dates.map((row1, index1) => ( 
                      <th style={{backgroundColor:"#7367f0",minWidth:"200px",fontWeight:"500"}}>{row1.MovementDate}</th>
                        ))}  
            ...
            <tbody style={{textAlign:"center"}}>
              {form.values.CheckList && form.values.CheckList.map((row, index) => ( 
                <tr style={{height:"44px"}}>
                   ...
                   <td>{row.PlanQty}</td> 
                   <td>{row.ActualQty}</td> 
                   <td>{row.AchievedPercent}</td> 
                   <td>{row.Balance}</td> 
                   
                   {form.values.Dates && form.values.Dates.map((row1, index1) => ( 
                     <>
              
                    {form.values.ActualData && form.values.ActualData.filter(filterrow => {
                        return row.fromwarehouseid==filterrow.WarehouseId &&
                        row.fromplantid==filterrow.PlantId &&
                        row.fromlotid==filterrow.LotId &&
                        row.wheatvarityid==filterrow.WheatVarietyId &&
                        filterrow.MDate==row1.MovementDate;
                      }).map((row2, index2) => ( 
                      <>
                      {row2.MDate==row1.MovementDate &&  
                      row.fromwarehouseid==row2.WarehouseId &&
                      row.fromplantid==row2.PlantId &&
                      row.fromlotid==row2.LotId &&
                      row.wheatvarityid==row2.WheatVarietyId &&
                      <td>{row2.MovementQty}</td> }
...
return (
    <Fragment>
      ...
      <CardComponent  header="Target Vs Actual"> 
        
     <TargetVsActual form={form}  onSubmit={onSubmit}   />
...
export default TargetVsActualData
