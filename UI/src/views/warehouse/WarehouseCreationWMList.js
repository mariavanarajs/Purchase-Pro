...
const WarehouseCreationWMList = () => {
  ...
  let title="Warehouse Creation - Lot Creation Team List";
  console.log(title);
  ...
  let actionRendorer=(row) => {
    ...
    let tx = "View";
          ...
          else if(row.approval_status==5 || row.approval_status==105)
          {
            tx="Lot Creation";
          }
      ...
      <Button.Ripple
        ...
        onClick={() => {
          if(row.approval_status==-1)
          {
            history.push(`/warehouse/masters/new_warehouse:` + row.wh_refid );
          }
          else if(row.approval_status==4 || row.approval_status==104)
          {
  ...
  return (
      ...
      <Card>
        ...
        <CardBody>
          <TableComponent columns={columns} url={url} formType="WarehouseCreationWMList" />
        </CardBody>
      </Card>
    </div>
    </Fragment>
