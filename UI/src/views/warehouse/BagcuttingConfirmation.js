...
const Bagcuttingconfirmation = ({form})  => { 
...
const ActionEntry = (BagCuttingId,Remarks,Status) => {
  ...
  apiPostMethod(apiBaseUrl + "warehouse/bagcutting/updateBagcut", postdata)
    ...
    if(UsrId==-5){
      ...
      history.push("/warehouse/BagcuttingConfirmation");
     // window.location.reload();
...
<div style={{Width:"970px",minHeight:"40vh",fontSize:"12px",overflowX:"auto"}} >
        <table className='table-sm'> 
            ...
            {form.values.CheckList && form.values.CheckList.map((row, index) => ( 
                   ...
                   onChange={(e) => onTextChange(e,row.bagcuttingid,form.values.CheckList,"ACRemarks")} 
                    form={form} id={`ACRemarks_${index}`} 
                    type="text" value={row.ACRemarks}   /></td>  
                          <td>{row.approvestatusName}</td>
                          <td>
                            <Button.Ripple color="primary" type="Button" onClick={(e) => {ActionEntry(row.bagcuttingid,row.ACRemarks,3);}}>Approve</Button.Ripple>
...
const BagcuttingaConfirmaionData = () => { 
  const history = useHistory();
...
approvestatus:formData.approvestatus.value,
    ...
    return(
        <Fragment>
        <CardComponent  header="Warehouse Bag Cutting Confirmation Screen">
       <Bagcuttingconfirmation form={form}  onSubmit={onSubmit}  />
     </CardComponent>
      </Fragment>
    )
...
export default BagcuttingaConfirmaionData 
