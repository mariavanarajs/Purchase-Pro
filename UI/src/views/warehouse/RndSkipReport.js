...
const RndSkipReport = ({form,onSubmit,ImgData,setImgData}) => { 
 ...
 const ActionEntry = (sub_lot_id,index,Status) => {
  if(!form.values.CheckList[index].RndSkipRemarks){ errorToast("Enter Remarks"); return false}
  let Data={
    RndSkipRemarks:form.values.CheckList[index].RndSkipRemarks,
    RndSkipFlag:1
...
apiPostMethod(apiBaseUrl + "warehouse/fumigation/getSublotFumgtnSkipDetail", fdata)
.then((response) => {
   const { data } = response;
   console.log("Response Data :: "+JSON.stringify(response));
   if (data.success) {
      ...
      <span id='warehouseid_Error' style={{color: 'red'}} ></span>
        ...
        <Col md="3" sm="12"> 
          ...
          <FormGroup className="d-flex justify-content-end mb-0">
            <Button.Ripple color="primary" type="Button" onClick={getSubLotDetailsRnd}>Show</Button.Ripple>
            &nbsp;&nbsp;&nbsp;
                ...
                <tr style={{height:"44px"}}>
                  ...
                  <input style={{fontSize:"12px",height:"30px",marginBottom:"-10px"}}
                   ...
                   onChange={(e) => onTextChange(e,row.sub_lot_id,form.values.CheckList,"RndSkipRemarks",index)} 
                    ...
                    type="text" value={row.RndSkipRemarks}  disabled/>

                  </td>

                  {/* <td> <Button.Ripple color="primary" type="Button" onClick={(e) => {ActionEntry(row.sub_lot_id,index,1);}}>Approve</Button.Ripple></td> * /}
...
return (
    <Fragment>
      ...
      <CardComponent  header="R&amp;D Skip Report"> 
        
     <RndSkipReport form={form}  onSubmit={onSubmit}  ImgData={ImgData} setImgData={setImgData}  />
...
export default RndSkipReportData
