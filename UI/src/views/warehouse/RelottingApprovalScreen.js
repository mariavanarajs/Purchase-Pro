...
function RelottingApprovalScreenFrom({form,onSubmit}) {
...
WeightmentSlip:data.results[0].WeightmentSlip,
BeforeImage:data.results[0].BeforeImage,
AfterImage:data.results[0].AfterImage,
...
const onActionClick = (RelotId) => {
		
 let Data={RelotStatus:'4',RelottingVendorId:form.values.RelottingVendor.value,
 RelottingCharges:form.values.RelottingCharges
}
  const postdata = {
    id:RelotId,
    ScreenType:'RETLOTAPPROVAL',
    Data
  }
  ...
  apiPostMethod(apiBaseUrl + "warehouse/relot/updateRelot", postdata)
    ...
    if(UsrId==-5){
      ...
      history.push("/warehouse/RelottingApproval");
     // window.location.reload();
  ...
  const onActionClickReject = (RelotId, Status) => {
...
<CustomTextInput label={'Relotting Charges'} form={form} id='RelottingCharges'   />
...
<>
                     {/* <Uploader
                        ...
                        label={"Wheighment Slip"}
                        title="Wheighment Slip"
                        id={"WeightmentSlip"}
                     ...
                     */}
 <Uploader isReadOnly={true} label={"Wheighment Slip"} selectedFileName={form.values.WeightmentSlip} />
...
<>
                     ...
                     */}
                      <Uploader isReadOnly={true} label={"Before Photo"} selectedFileName={form.values.BeforeImage} />
...
<>
                    ...
                    */}
                     <Uploader isReadOnly={true} label={"After Photo"} selectedFileName={form.values.AfterImage} />
                       </>
...
<Button.Ripple color="danger" type="Button" style={{bottomMargin:"5px",rightMargin:"10px"}} onClick={(e) => {onActionClickReject(RelotId, -1);}}>Reject</Button.Ripple>
</div>
<Col className="offset-md-6 d-md-flex justify-content-end">
<div class="p-1">
<Button.Ripple color="primary" type="Button" style={{bottomMargin:"5px"}}
onClick={(e) => onActionClick(RelotId)} >Submit</Button.Ripple>
</div>
<div class="p-1 mx-2">
...
<CardComponent header="Relotting - Entry Screen	">
 <RelottingApprovalScreenFrom form={form}onSubmit={onSubmit}/>
 </CardComponent>
 </Fragment>
)
}

export default RelottingApprovalScreen
