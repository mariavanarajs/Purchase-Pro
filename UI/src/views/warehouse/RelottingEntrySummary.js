...
const RelottingEntrySummaryForm = ({ form,onSubmit }) => {
		...
		},
    {
      ...
      if(RelotStatus==-1){
        history.push("/warehouse/RelottingRequest:" + RelotId);
      }else{
        history.push("/warehouse/RelottingEntryScreen:" + RelotId);
      }
  ...
  return (
    <Fragment>
       <Card>
        <CardHeader>
          <CardTitle>Relotting – Entry Screen </CardTitle>
        </CardHeader>
        <CardBody>
          <TableComponent columns={columns} url={RelottingUrl} formType="getRelottingforEntry" />
        </CardBody>
      </Card>
...
const RelottingEntrySummary = () => {
  ...
  export default RelottingEntrySummary;
