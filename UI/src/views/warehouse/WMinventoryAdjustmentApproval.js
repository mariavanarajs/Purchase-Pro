...
/*
  ...
  const UpdateItem = (Id,Status) => {
    let Physical_Qty = document.getElementById("Physical_Qty_"+Id).value;
    let allowed_diff_qty = document.getElementById("allowed_diff_qty_"+Id).value;
      alert("The Physical Qty is :"+ Physical_Qty);
      // alert(allowed_diff_qty);

      //Dijo
      // alert(Physical_Qty);
      // if(parseFloat(Physical_Qty)>parseFloat(allowed_diff_qty)){
      //   errorToast("Physical Qty beyond Approval Limit");
      //   return false;
      // }

      if(Status=="2"){
        Status="3"; //As Requested by client WMAdjustment Accounts Approval not required so directly move to approved status
      }
      ...
      console.log(Physical_Qty);
      let fdata = {
        Data,
      formType:"updateInventoryWMApprove"
      };
  ...
  let fdata = {
    stockEntryformData,
   formType:"WMInventoryAdjustmentApproval",
  };
    ...
    return ( 
        ...
        <Row > 
            <Col md="3" sm="12">  
            <DatePicker label={"From Date"} form={form} id="Fromdate" format="DD/MM/YYYY" isDateRange={false} type="date"  />
 
            </Col>
            <Col md="3" sm="12">  
            <DatePicker label={"To Date"} form={form} id="Todate" format="DD/MM/YYYY" isDateRange={false} type="date"  />
        ...
        <table className='table-sm'> 
            <thead className='bg-primary text-white' style={{height:"50px",fontWeight:"bold",textAlign:"center"}}> 
                <tr> 
                    <th style={{minWidth:"200px",fontWeight:"400"}}>Unique No.</th>
                    <th style={{minWidth:"200px",fontWeight:"400"}}>Posting Date</th>
                    <th style={{minWidth:"200px",fontWeight:"400"}}>Lot No</th>
                    ...
                    <th style={{minWidth:"200px",fontWeight:"400"}}>SAP Qty</th> 
                    <th style={{minWidth:"200px",fontWeight:"400"}}>Physical Qty</th> 
                    <th style={{minWidth:"200px",fontWeight:"400"}}>Up Qty</th>
                    <th style={{minWidth:"200px",fontWeight:"400"}}>Waiting At</th>
                    <th style={{minWidth:"200px",fontWeight:"400"}}>Overall Duration</th>
                    <th style={{minWidth:"200px",fontWeight:"400"}}>Screen Duration</th>
                      ...
                      stockEntryformData.length>0 &&
                        ...
                        <tr>
                          ...
                          <td>
                            ...
                            <CustomTextInput form={form} id={"Physical_Qty_"+item.Physical_Inventory_Id} type="text" value={item.Physical_Qty} 
                            onChange={(e) => CalcQty(e,item.Physical_Inventory_Id)} disabled/>
                            <CustomTextInput form={form} disabled id="wheatqty" type="hidden" value={item.wheatqty}/>
                            <CustomTextInput form={form} disabled id="SAP_Qty" type="hidden"  value={item.SAP_Qty}/>
                            <CustomTextInput form={form} disabled id="init_lot_qty" type="hidden"  value={item.init_lot_qty}/>
                            <CustomTextInput form={form} id="allowed_diff_in_percent" type="hidden" value={item.allowed_diff_in_percent} />
                            <CustomTextInput form={form} id={"allowed_diff_qty_"+item.Physical_Inventory_Id} type="hidden"  value={item.allowed_diff_qty} />
                            <CustomTextInput form={form} id="totalcapacity" type="hidden" value={item.totalcapacity} />
                          </td> 
                          ...
                          <td>
                            <Button.Ripple color="primary" type="Button" style={{bottomMargin:"5px"}} onClick={(e) => {UpdateItem(item.Physical_Inventory_Id,2);}}>Approve</Button.Ripple>
                            &nbsp;<Button.Ripple color="primary" type="Button" onClick={(e) => {UpdateItem(item.Physical_Inventory_Id, -1);}}>Reject</Button.Ripple>
                            </td>                        </tr>
                        )
