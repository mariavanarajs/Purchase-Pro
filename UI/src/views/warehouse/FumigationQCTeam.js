    ...
    const onTextChange = (e,PKey, CheckList,Val) => {
      ...
      for(let i=0;i<CheckList.length;i++){
        if(CheckList[i].FumigationId==PKey){
            if(Val=="DegasStatus"){
              CheckList[i].sDegasStatus=e.value;
              CheckList[i].sDegasStatuslabel=e.label;
            }
...
const ActionEntry = (sub_lot_id,index,Status) => {
    ...
    if(!form.values.CheckList[index].sDegasStatus){ errorToast("Select Status"); return false}
    //let Data=form.values.CheckList[index];
  let Data={
    Status:form.values.CheckList[index].sDegasStatus,
  }
  const postdata = {
   
    ScreenType:'UPDATEFUMIGATION_QC',
    Data,
    sub_lot_id:form.values.CheckList[index].SublotId,
...
const gotoDegasQC= () =>{
  history.push("/warehouse/FumigationDegasQC");
}
...
<Fragment>
<Row>
  <Col md="12" sm="12" style={{width:"1110px",height:"300px",overflowY:"auto",fontSize:"12px",overflowX:"auto"}}>
        ...
        <table className='table-sm'> 
            <thead className='bg-primary text-white ' style={{height:"50px",textAlign:"center"}}> 
                <tr> 
                   ...
                   <th style={{minWidth:"150px"}}>Plant</th>
                    ...
                    <th style={{minWidth:"150px"}}>Last Fumigation Type</th>
                    ...
                    <th style={{minWidth:"150px"}}>Reason For Delay</th> 
                    <th style={{minWidth:"150px"}}>Bag Type</th> 
                    <th style={{minWidth:"150px"}}>Fumigation Type</th> 
                    <th style={{minWidth:"150px"}}>Fumigation Agency</th> 
                    <th style={{minWidth:"150px"}}>Fumigator Name	</th> 
                    <th style={{minWidth:"150px"}}>Vendor Name	</th> 
                    <th style={{minWidth:"150px"}}>Amount</th> 
                    <th style={{minWidth:"150px"}}>ALP Count</th> 
                    <th style={{minWidth:"150px"}}>Degas Status</th> 
            ...
            {form.values.CheckList && form.values.CheckList.map((row, index) => ( 
                        <tr>
                          ...
                          <td>{row.Fumigation_Type == 'ALP 9GMS' ? 'ALP' : 'Quick Gas'}</td>
                      ...
                      <td>{row.fumigationstatus}</td>
                       <td>
                     {row.ReasonDelayStatus}
                         </td>  
                         <td>
                         {row.BAG_NAME}
                         </td>  
                         <td>
                         {row.Fumigation_Type == 'ALP 9GMS' ? 'ALP' : 'Quick Gas'}
                         </td>  
                         <td>
                       {row.Fumigation_Agency}
                         </td>  
                         <td>
                        {row.Fumigator_Name}
                         </td>  
                      ...
                      {row.Vendor_Name}
                         ...
                         <td>{row.ALPCount}</td>
                         <td>
                         {row.sDegasStatus && row.sDegasStatuslabel}
                         {!row.sDegasStatus &&  <CustomDropdownInput  url={`${apiBaseUrl}warehouse/master/getDegassStatusList`} 
           form={form} id={`DegasStatus_${index}`} 
           onChange={(e) => onTextChange(e,row.FumigationId,form.values.CheckList,"DegasStatus",index)} 
            />}
                         </td>
                      <td>
                     <Button.Ripple color="primary" type="Button" 
                     onClick={(e) => {ActionEntry(row.FumigationId,index,1);}}>SAVE</Button.Ripple>
                            </td> 
...
approvestatus:formData.approvestatus.value,
    ...
    return(
        <Fragment>
        <CardComponent  header="Degasing Confirmation Entry - QC Team">
       <FumigationQCTeam form={form}  onSubmit={onSubmit}  />
     </CardComponent>
      </Fragment>
    )
...
export default FumigationQCTeamData 
