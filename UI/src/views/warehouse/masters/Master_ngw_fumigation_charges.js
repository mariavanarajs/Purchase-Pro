...
const Master_ngw_fumigation_chargeform = ({ form,onSubmit }) => {
    ...
    apiPostMethod(apiBaseUrl + "warehouse/master/getMaster_ngw_fumigation_typeById", fdata)
      .then((response) => {
        ...
        if (data.success) {
          ...
          form.setFieldValue("Status", {  label: data.results[0].Status == 1 ? 'ALP' : 'Quick Gas',value: data.results[0].Status });
...
const Master_ngw_fumigation_charges = () => {
    ...
    return (
      <Fragment>
        <CardComponent header="Master Fumigation Type">
          <Master_ngw_fumigation_chargeform form={form}  onSubmit={onSubmit} />
        </CardComponent>
      </Fragment>
    );
  };
  export default Master_ngw_fumigation_charges;
