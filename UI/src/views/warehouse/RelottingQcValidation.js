...
////import RelottingQCValidation from "./RelottingQCValidation";
import { Row, Col } from "reactstrap";
import { CustomDropdownInput, CustomTextInput } from "../forms/custom-form";

import { RelottingUrl } from "../../urlConstants";
const RelottingQCValidation = ({ form,onSubmit }) => {
		...
		},
    {
	  ...
	  const ViewRelot = (RelotId) => {
		  history.push("/warehouse/ViewRelottingQcValidation:" + RelotId);
	  }
  ...
  const RefreshPage = () => {
    history.push(`/warehouse/masters/RelottingQcValidation`);
  };
  ...
  return (
    <Fragment>
       <Card>
        <CardHeader>
          <CardTitle>Relotting - QC Validation</CardTitle>
        </CardHeader>
        <CardBody>
          <TableComponent columns={columns} url={RelottingUrl} formType="getRelotting" />
        </CardBody>
      </Card>
...
const RelottingQcValidation = () => {
      ...
      .then((response) => {
        ...
        if(UsrId==-5){
          ...
          if(RespId && RespId>=1)
          {
            ...
            {
              history.push("/warehouse/masters/RelottingQcValidation:0");
            }
            else
            {
              history.push("/Warehouse/Masters/RelottingQcValidation");
  ...
  export default RelottingQcValidation;
