...
const WarehouseCreationWMList = () => {
  
  let title="Warehouse Creation - Wheat Movement Team List";
  console.log(title);
  ...
  let actionRendorer=(row) => {
    ...
    let tx = "View";
          ...
          if(row.approval_status=='6' || row.approval_status=='106')
          {
            tx="Lot Approve";
          }
      ...
      <Button.Ripple
        ...
        onClick={() => {
          ...
          else if(row.approval_status=='1' || row.approval_status=='101' )
          {
          history.push(`/warehouse/wclwmmgrapprove:` + row.wh_refid );
          }
          else if(row.approval_status=='6' || row.approval_status=='106')
          {
          history.push(`/warehouse/wcllotmgrapprove:` + row.wh_refid );
          }
  ...
  return (
      ...
      <Card>
        ...
        <CardBody>
          <TableComponent columns={columns} url={url} formType="WarehouseCreationWMMgrList" />
        </CardBody>
      </Card>
    </div>
    </Fragment>
...
export default WarehouseCreationWMList;
