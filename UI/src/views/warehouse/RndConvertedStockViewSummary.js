...
const RndConvertedStockViewSummary = ({form,onSubmit}) => { 
    ...
    const ViewRndLot = (warehouseid,plantid,lotid, sub_lot_id,wheatvarietyid,QaNo) => {
  
      //history.push("/warehouse/RndConvertedStockView:"+warehouseid+"/"+plantid+"/"+lotid+"/"+sub_lot_id+"/"+wheatvarietyid);
      history.push("/warehouse/RndConversionEntryView/"+warehouseid+"/"+plantid+"/"+lotid+"/"+sub_lot_id+"/"+wheatvarietyid+"/"+QaNo);
  }
 //  Dijo 01
...
const fileUploadAction = (id) => {
 ...
 const FillWheatVarityList = (StorageLocationId) => {
  //let fdata = { WareHouseId:form.values.warehouseid.value,PlantId, screenType: "RND" };
 ...
 const ShowStock = (e) => {
  ...
  return (
    <Fragment>
        ...
        <Col md='12' sm="12"> 
          <div className='d-flex  justify-content-end'>
            <CustomTextInput label={"Search"} form={form} id="Search" type="search" 
              onChange={(e) => showWarehousewisestocks(e)}
            />
          </div>
        </Col> 
      </Row> 
      ...
      <Row> 
        {/* <Col md='12' sm="12" style={{width:"1110px",height:"520px",overflowX:"scroll",overflowY:"scroll",fontSize:"12px", sortable: true}}>  */}
        <Col md='12' sm="12"> 
        <TableComponent showDownload hideSearch columns={columns} data={form.values.CheckList}/>  
        
        {/*
        <div>
        <table id="TableID" className='table-sm' style={{float:"left",width:"100%"} } > 
            <thead className='bg-primary text-white' style={{height:"50px", top:"0px",textAlign:"center"}}>
                <tr>
                    <th >Lot No</th> 
                    <th >QA No</th> 
                    <th >Wheat Variety</th> 
            ...
            <tbody style={{textAlign:"center"}}>
              {form.values.CheckList && form.values.CheckList.map((row, index) => ( 
                <tr style={{height:"44px"}}>
                  ...
                  <td>
                    <Button.Ripple color="primary" type="Button" 
                      onClick={(e) => {ViewRndLot(row.warehouseid, row.plantid, row.lotid, row.sub_lot_id,row.wheatvarietyid,row.qano);}}>
                        View 
                    </Button.Ripple>
                  </td> 
                </tr>
...
const RndConvertedStockViewSummaryData = () => { 
    const history = useHistory();
...
return (
    <Fragment>
      ...
      <CardComponent  header="R&amp;D Lot Converted Stock View"> 
        
     <RndConvertedStockViewSummary form={form}  onSubmit={onSubmit}   />
...
export default RndConvertedStockViewSummaryData
