...
const RndConvertedStockLineItemView = ({form,onSubmit}) => { 
    ...
    //console.log("Request Url :: "+apiBaseUrl + "Master/getstockEntrydatabyid", fdata);
     apiPostMethod(apiBaseUrl + "warehouse/RndlotConversion/getRndLotParameterDet", fdata)
     .then((response) => {
       const { data } = response;
    ...
    const ViewRndLot = (sub_lot_id) => {
      ...
      history.push("/warehouse/RndConvertedStockLineItemView:"+sub_lot_id);
  }
...
const fileUploadAction = (id) => {
 ...
 const FillWheatVarityList = (StorageLocationId) => {
  let fdata = { WareHouseId:form.values.warehouseid.value,StorageLocationId,PlantId:form.values.locationid.value, screenType: "RND" };
  ...
  return (
    <Fragment>
      ...
      <Row>
        <Col md="3" sm="12">      
          <CustomDropdownInput  
            label={"Storage Location"} id={"storagelocationid"}
            form={form} 
        ...
        <Col sm="12"> 
          <FormGroup className="d-flex justify-content-end mb-0">
            <Button.Ripple onClick={onFetchStockentryById} color="primary" type="Button">Show</Button.Ripple>&nbsp;&nbsp;&nbsp;
            <Button.Ripple color="primary" type="Button" style={{width:"110px"}} onClick={ExcelExport}>Excel Export</Button.Ripple>  
          </FormGroup>
        </Col>
      </Row>
      <Row> 
        <Col md='12' sm="12"> 
          <div className='d-flex  justify-content-end'>
            <CustomTextInput label={"Search"} form={form} id="Search" type="search"
      ...
      <Row > 
        <Col md="12" sm="12" style={{width:"1110px",height:"520px",overflowX:"scroll",overflowY:"scroll",fontSize:"12px"}}>
              
        <div>
        <table id="TableID" className='table-sm' > 
            <thead className='bg-primary text-white' style={{height:"50px",fontSize:"12px",textAlign:"center", position:"sticky", top:"50px", insetBlockStart:0}} > 
                <tr> 
                    <th style={{minWidth:"200px",fontWeight:"500"}} >QA No</th> 
                    <th style={{minWidth:"200px",fontWeight:"500"}}>Warehouse</th> 
                    ...
                    <th style={{minWidth:"200px",fontWeight:"500"}}>R&D Date</th> 
                   <th style={{minWidth:"200px",fontWeight:"500"}}>Wheat Variety</th> 
                   <th style={{minWidth:"200px",fontWeight:"500"}}>Stock in MTS</th> 
                   
                   {form.values.Parameters && form.values.Parameters.map((row1, index1) => ( 
                      <th style={{minWidth:"200px",fontWeight:"500"}}>{row1.parametername}</th> 
                    ))}  
                  </tr>
            ...
            <tbody style={{textAlign:"center"}}>
              {form.values.CheckList && form.values.CheckList.map((row, index) => ( 
                <tr style={{height:"44px"}}>
                   ...
                   <td>{row.keyloandoqty}</td> 
                   {form.values.Parameters && form.values.Parameters.map((row2, index2) => ( 
                     <>
                     <td>
                       {form.values.ParameterDetails && form.values.ParameterDetails.map((row3, index3) => ( 
                         <>
                     {row3.rnd_lot_conversion_id==row.rnd_lot_conversion_id && row2.rnd_lot_parametermasterid==row3.ParamId 
                && row3.parametertype=="1" && <>{row3.ParamValue}</> }

{row3.rnd_lot_conversion_id==row.rnd_lot_conversion_id && row2.rnd_lot_parametermasterid==row3.ParamId 
                && row3.parametertype=="2" && row3.Attachment!="" && <><Uploader isReadOnly={true} selectedFileName={row3.Attachment} /></> }
                        </> 
                      ))}
                      </td>  
                      </> 
...
const RndConvertedStockLineItemViewData = () => { 
    const history = useHistory();
...
return (
    <Fragment>
      ...
      <CardComponent  header="R&amp;D Lot Converted Stock Live View"> 
        
     <RndConvertedStockLineItemView form={form}  onSubmit={onSubmit}   />
...
export default RndConvertedStockLineItemViewData
