...
const RndConvertedStockView = ({form,onSubmit}) => { 
    ...
    const onFetchStockentryById = (SearchVal) => {
      let fdata = {
        id:refid,
        search:SearchVal
      };
    ...
    console.log("Request Url :: "+apiBaseUrl + "Master/getstockEntrydatabyid", fdata);
     apiPostMethod(apiBaseUrl + "warehouse/RndlotConversion/getRndLotconversionDet", fdata)
     .then((response) => {
       const { data } = response;
    ...
    const ViewRndLot = (sub_lot_id) => {
      ...
      history.push("/warehouse/RndConvertedStockView:"+sub_lot_id);
  }
...
const fileUploadAction = (id) => {
 ...
 const FillWheatVarityList = (PlantId) => {
...
// console.log("Request Url :: "+apiBaseUrl + "warehouse/getbagcuttingEntrydatabyid", fdata);
 ...
 const SetSearch = (e)=>{
  // form.setValues({"search":e.target.value});
   onFetchStockentryById(e.target.value);
 }
 const ShowStock = (e) => {
   ...
   .catch((error) => {
    ...
    return (
        ...
        <Row> 
            ...
            onChange={(e) => SetSearch(e)} type="search"  />
            </div>
            </Col> 
        ...
        <table id="TableID" className='table-sm' style={{float:"left",width:"100%"}} > 
            <thead className='bg-primary text-white' style={{height:"50px",textAlign:"center"}} > 
                ...
                <tr> 
                    ...
                    <th >Wheat Variety</th> 
                    <th >R&D Date</th> 
                    <th >R&D Confirmed Qty</th> 
                    <th >Fumigation Cleared Qty</th> 
                    <th >Key Loan Qty</th> 
            ...
            <tbody style={{textAlign:"center"}}>
              {form.values.CheckList && form.values.CheckList.map((row, index) => ( 
                <tr style={{height:"44px"}}>
                   ...
                   <td>{row.RNDDate}</td> 
                   <td>{row.rndconfirmqty}</td> 
                   <td>{row.fumigationclearedqty}</td> 
...
const RndConvertedStockViewData = () => { 
    const history = useHistory();
...
return (
    <Fragment>
     ...
     <RndConvertedStockView form={form}  onSubmit={onSubmit}   />
...
export default RndConvertedStockViewData
