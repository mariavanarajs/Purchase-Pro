...
const TruckArrivalRedirct = ({ onAdded,results }) => {
  ...
  useEffect(() => {
    ...
    CONTAINER_NO:results.CONTAINER_NO,
    IDNLF:results.IDNLF,
  ...
  const onAddTruckDetails = (status) => {
    ...
    let fdata = { ZQTY: results.ZSUPPLIER_INV_QTY || 0, 
      ...
      formType: "A", 
       screenType: 'SDI'  , 
       TRUCK_NO:results.TRUCK_NO, ZPO_NUMBER:formData.ZPO_NUMBER , PO_LINE_ITEM:formData.PO_LINE_ITEM , ZSUPPLIER_CODE:formData.ZSUPPLIER_CODE , CONT_NO:formData.CONTAINER_NO , DRIVER_NO:formData.DRIVER_NO ,ZSUPPLIER_NAME:formData.ZSUPPLIER_NAME,IDNLF:formData.IDNLF};
