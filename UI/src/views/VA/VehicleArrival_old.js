...
import { Card, CardHeader, CardTitle, CardBody, Button, FormGroup, Row, Col, Label, Input } from "reactstrap";
  ...
  function handleChange(selectedOption) {
    ...
    apiPostMethod(apiBaseUrl + `GatePro/Master/getFgSalesReturnInfo/${vehicleNo}`)
      ...
      const getLoadingAndUnloadingInfo = () => {
        ...
        apiPostMethod(apiBaseUrl + `GatePro/Master/getLoadingAndUnloadingInfo/0/${id}/${UserDetails.USERID}`)
          .then((response) => {
            const { data } = response;
            if (data.success == true) {
              setData(data.results[0]);
    ...
    <div>
      <Card>
        ...
        <CardBody>
            ...
            {(VEHICLE_TYPE || vehicle_type || SCREEN_TYPE || moduleType || returnRefNo) &&
              <Col md="4" sm="12">
                <FormGroup>
                  ...
                  <Input type="text" disabled={true} value={VEHICLE_TYPE === 'Truck' ? 'Purchase' : VEHICLE_TYPE === 'Container' ? 'Purchase' : VEHICLE_TYPE === 'Rake' ? 'Purchase' : vehicle_type === 'Rake' ? 'Purchase' : SCREEN_TYPE == "EVADP" ? 'IAS' : returnRefNo != '' ? 'FG-SALES RETURN' : moduleType != '' ? moduleType : SCREEN_TYPE} placeholder={"Select the PO Number"} />
                  <span id="ZPO_NUMBER_Error" style={{ color: "red" }} ></span>
                </FormGroup>
              </Col>}
          </Row>
          ...
          {moduleType == 'FG-STO' ? <STOGateIn data={data} setModuleType={setModuleType} setSelectedValue={setSelectedValue} getUnLoadingData={getUnLoadingData} Unloading_Gate_in_Vehicle={Unloading_Gate_in_Vehicle}/> : null}
          {returnRefNo != '' ? <FGReturnGateIn data={data} setReturnRefNo={setReturnRefNo} setSelectedValue={setSelectedValue} setData={setData} getUnLoadingData={getUnLoadingData} Unloading_Gate_in_Vehicle={Unloading_Gate_in_Vehicle}/> : null}
          {moduleTypeId == 6 ? <SSAndPMUnloadingGateIn data={data} setReturnRefNo={setReturnRefNo} setSelectedValue={setSelectedValue} setModuleTypeId={setModuleTypeId} getUnLoadingData={getUnLoadingData} Unloading_Gate_in_Vehicle={Unloading_Gate_in_Vehicle} setModuleType={setModuleType}/> : null}
          {data?.moduleTypeId == 9 ? <RmReturnGateIn data={data}/> : null}
          {moduleType == 'Water (Tanker)' ? <RmWaterGateIn data={data} setSelectedValue={setSelectedValue} setData={setData} setModuleType={setModuleType} getUnLoadingData={getUnLoadingData}/> : null}
          {moduleType == 'S&S Purchase' ? <SSAndPMPurchaseGateIn data={data} setSelectedValue={setSelectedValue} setData={setData} setModuleType={setModuleType} getUnLoadingData={getUnLoadingData}/> : null}
        </CardBody>
      </Card>
      {/* <TruckListTable postData={filter} actionCell={actionsCol} ScreenName={"Unloading"} title={"Vehicle List"} /> */}

      <TruckDetails loadingData={unloadingData} getLoadingData={getUnLoadingData} />
