...
const GateInOutReport = ({ actionRendorer }) => {
    ...
    const getGateInOutReport = () => {
        ...
        showLoader();
        apiPostMethod(apiBaseUrl + `GatePro/Report/WeighmentReport/${fromDateMilliSecond}/${toDateMilliSecond}/${moduleTypeId}/${plantId}/${waitingStatusId}/${UserDetails.USERID}`)
            .then((response) => {
                const { data } = response;
