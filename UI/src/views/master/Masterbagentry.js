...
import { master_bagUrl } from "../../urlConstants";
const Master_bagform = ({ form,onSubmit }) => {
  ...
  const onFetchSDIdetailsByBAG_REFID = () => {
    ...
    showLoader();
    //alert("ok")
    apiPostMethod(apiBaseUrl + "Master/getMasterBagDetailsByRefId", fdata)
      .then((response) => {
        const { data } = response;
        ...
        actionRendorer={(row) => {
          ...
          return (
            <Button.Ripple
              color="primary"
              onClick={() => {
                history.push(`/master/master_bag:` + row.BAG_REFID );
              }}
            >
              {tx}
            </Button.Ripple>
  ...
  const form = useFormik({
    ...
    validationSchema: Yup.object().shape({
     ...
     BAG_CODE: validation.required({ message:"Bag code should not be empty", isObject: false }),
      ...
      WEIGHT: validation.number({  message:"Weight should be numeric value",isObject: false, decimalAllowed:true }),
    }),
    onSubmit(values) {},
  });
 ...
 // alert("1");
  ...
  //  alert(JSON.stringify(postdata)) ;
   ...
   console.log(JSON.stringify(postdata))
    ...
    showLoader();
    apiPostMethod(apiBaseUrl + "Master/updatemaster_bag1", postdata)
      .then((response) => {
      //  alert("4");
        const { data } = response;
        console.log(JSON.stringify(response))
        ...
        if(RespId && RespId>=1)
        {
          ...
          {
            history.push("/master/master_bag:0");
          }
          else
          {
            history.push("/master/master_bag");
