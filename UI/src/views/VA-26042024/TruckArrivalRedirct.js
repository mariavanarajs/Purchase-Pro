...
const TruckArrivalRedirct = ({ onAdded,results }) => {
  ...
  useEffect(() => {
    ...
    ZPO_NUMBER:results.ZPO_NUMBER,
    PO_LINE_ITEM:results.PO_LINE_ITEM,
    DRIVER_NO:results.DRIVER_NO,
    ZSUPPLIER_CODE:results.ZSUPPLIER_CODE,
    ZSUPPLIER_NAME:results.ZSUPPLIER_NAME,
    BROCKER_NAME:results.ZVENDOR_CODE,
    BROCKER_CODE:results.ZVENDOR_NAME,
  ...
  const onAddTruckDetails = (status) => {
    ...
    let fdata = { ZQTY: results.ZSUPPLIER_INV_QTY || 0, 
      ...
      formType: "A", 
       screenType: 'SDI'  , 
       TRUCK_NO:results.TRUCK_NO, ZPO_NUMBER:formData.ZPO_NUMBER , PO_LINE_ITEM:formData.PO_LINE_ITEM , ZSUPPLIER_CODE:formData.ZSUPPLIER_CODE , CONT_NO:formData.TRAILER_NO , DRIVER_NO:formData.DRIVER_NO ,ZSUPPLIER_NAME:formData.ZSUPPLIER_NAME,IDNLF:formData.IDNLF};
  ...
  apiPostMethod(apiBaseUrl + "RakeloadingController/Redirect_Loading_Update", {id:results.PI_REFID})
    .then((response) => {
  ...
  const onAddTruckDetail = (status) => {
    ...
    let fdata = { ZQTY: results.ZSUPPLIER_INV_QTY || 0, 
      VECHICAL_STATUS: status, 
      formType: "A", 
       screenType: results.SCREEN_TYPE, 
       TRUCK_NO:results.TRUCK_NO, ZPO_NUMBER:formData.ZPO_NUMBER , PO_LINE_ITEM:formData.PO_LINE_ITEM , ZSUPPLIER_CODE:formData.ZSUPPLIER_CODE , CONT_NO:results.TRAILER_NO , DRIVER_NO:formData.DRIVER_NO ,ZSUPPLIER_NAME:formData.ZSUPPLIER_NAME,IDNLF:formData.IDNLF};
    showLoader();
    apiPostMethod(vaUrl, fdata)
      .then((response) => {
        ...
        if (data.success) {
          ...
          apiPostMethod(apiBaseUrl + "RakeloadingController/Redirect_Loading_Update", {purchase_info_id:rake_data,id:results.PI_REFID})
            .then((response) => {
          ...
          apiPostMethod(vaUrl, {status: 35,formType: "U",id:results.PI_REFID})
        .then((response) => {
          const { data1 } = response;
          if (response.data.success == 1) {
...
export default TruckArrivalRedirct;
