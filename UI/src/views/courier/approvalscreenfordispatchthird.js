...
const ConsignmentPaymentInitiate = ({ title, url, actionRenderer }) => {
  ...
  const loadTableData = async () => {
    ...
    showLoader();
    try {
      const response = await apiPostMethod(apiBaseUrl + "CourierMaster/getpaymentdetails3", postdata);
      const { data } = response;
  ...
  const handleApproveConfirmation = async (row) => {
    ...
    try {
      const response = await apiPostMethod(apiBaseUrl + "Sap/CourierpaymentAPIController/courierpaymentapiforSAP", postdata);
      const { data } = response;

      if (data.success == 1) {
        ShowToast(data.results);
  ...
  const handleRejectConfirmation = async (row) => {
    ...
    showLoader();
    try {
      const response = await apiPostMethod(apiBaseUrl + "CourierMaster/rejectCourierItem3", postdata);
      const { data } = response;
