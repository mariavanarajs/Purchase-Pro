...
const QualityCheck = ({ isViewOnly, title, returnUrl, status }) => {
  ...
  const tabs = [
    ...
    },
    {
      ...
      title: "Silo To Mill",
      ShowTab:SiloExist,
      renderTab: () => <STMListTable postData={tableFilter1} actionCell={actionsCol_STM} title={title || "Quality Check"} ScreenName={"Quality Check"} hideFilter={isViewOnly} />,
    },
  ];
  ...
  const tableFilter1 = {
    ...
    //vehicleStatus: status || "2,3,4,5,6,24,15",
    status: status || "2",
    otherfilter: {
      from: getFromDate(values.date),
      to: getToDate(values.date),
